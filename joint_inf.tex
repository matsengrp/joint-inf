...
\end{equation}
%EM ancestral state splits not introduced-- can we just say "ancestral states"? Note that splits appear below as well.
%das: I removed "splits" where I could, but kept it right before the theorem. Thoughts?
...
%EM This is an important paragraph but the meat is hidden in the middle.
%EM We need to start with a topic sentence that says that For branch lengh, we find a systematic bias even in the short-branch case (Figure ref).
%EM Then we can talk about details such as discontinuities and how marginal works great.
%das: okay, reworked.
...
The estimates for $\hat{p}_w$ in this case look reasonable at first, compared with those estimates when $p_{y^*}$ is small and $p_{x^*}$ is large (Fig.~\ref{fig:bl-general-inconsistency}), though joint inference predictably underestimates the interior branch length.
...
%das: yeah, it was kind of hand-wavy.
...
%EM I suggest compacting the next few sentences.
...
%EM What is the "general case" here?
%das: did some scalpeling and rewording
