...
\end{align*}
for some $j$.
For the other term, we need to define
$$
\ancestralToSplit:\alphabet^\nAncestralStateRows\times\alphabet^\nSiteRows\rightarrow\mathcal{P}(\ancestralSplitSet)
$$
that takes both the ancestral state column and site column as input.
Given $\alignmentColumnRV$ as well as an ancestral state--valued random variable $\ancestralStateColumnRV$ define
$$
