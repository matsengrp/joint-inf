...
%EM Need a definition for \ancestralToSplit here.
$$
\ancestralToSplit:\alphabet^\nAncestralStateRows\times\alphabet^\nSiteRows\rightarrow\mathcal{P}(\ancestralSplitSet)
$$
as a mapping of ancestral states to ancestral state splits.
In this case, the ancestral states we consider depend on whether their corresponding tip labels had their complements taken or not: if we exclude a set of tip labels $\alignmentColumn$ from the set of site patterns by taking its complement, we let $\ancestralToSplit(\alignmentColumn, \ancestralStateColumn)$ be the complement of $\ancestralStateColumn$; otherwise $\ancestralStateColumn$ is unchanged.
We define
$$
...
\end{align*}
for some $j$ and $k$.
Then, the $i$th factor of \eqref{eq:full_likelihood} has the property
$$
P(\alignmentColumnRV=\alignmentColumn_i, \ancestralStateColumnRV=\ancestralStateColumn_i \mid \tau, t) \propto P(\siteSplitRV=\siteSplit_j, \tau, t) \cdot P(\ancestralSplitRV=\ancestralSplit_k \mid \siteSplitRV=\siteSplit_j, \tau, t).
$$
