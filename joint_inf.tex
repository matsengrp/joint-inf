...
\caption{
%EM I'm sure you're getting to this, but we need to change around wording to compensate for the use of probabilities vs. fidelities now.
%das: You just mean for the figure captions, right? I changed it in the figures and a few places in the text, but maybe you meant we should change around the whole text to be in terms of branch lengths? Should we change the InvFels/Fels tree figure, too?
    An analytically-derived region of topological inconsistency in terms of probabilities of a character change along a branch for ``perfect'' data generated on the InvFels topology (Fig.~\ref{fig:farris-fels-top}) with $p_{w^*} = p_{y^*}$ (in terms of fidelities, $w^*=y^*$).
...
As an example, Fig.~\ref{fig:bl-loose-inconsistency} shows the case where we have poor estimation of one of the three branch parameters when $x_L=x_U=y_L=y_U=0.1$.
...
This tendency to include $(1+w)$ terms in the likelihood results in a bias toward one of branch fidelities.
...
\caption{
    Analytically-derived region of branch parameter inconsistency in terms of probabilities of a character change with the same data-generating setup as Fig.~\ref{fig:inconsistency-farris}.
    In the marked region, either $\hat{p}_w=0$ or one of $p_{x^*}-.05 \le \hat{p}_x \le p_{x^*}+.05$ or $p_{y^*}-.05 \le \hat{p}_y \le p_{y^*}+.05$ will not be true, resulting in poor estimation of the pendant branch parameters.
    In terms of fidelities, these bounds are $x^*-0.1 \le \hat{x} \le x^*+0.1$ and $y^*-0.1 \le \hat{y} \le y^*+0.1$.
...
This region encompasses the majority of the branch parameter space; even given the correct topology and performing our best possible optimization, we have many situations where we will estimate the interior branch length to be zero.
...
\citet{Sagulenko2018-xl} explicitly warn their approximation is only valid for ``short'' branches, though Fig.~\ref{fig:bl-general-bias} shows even when branches are short there is a consistent bias.
