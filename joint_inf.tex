...
We will use $\hat{\fullAncestralStates}$ to denote the estimate for $\fullAncestralStates$ obtained by maximization \eqref{eq:profile_likelihood}.
...
We compute these maxima over a lattice in steps of $10^{-3}$ for $x$ and $y$ from $10^{-3}$ to $1-10^{-3}$---we do not include zero or one in our lattice to further stabilize the fits, recalling from Section~XXXX that cases where $x$ or $y$ are equal to zero or one can result in pathologies.
