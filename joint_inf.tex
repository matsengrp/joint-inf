...
For branch length estimation on the correct topology, we show that this joint method cannot correctly estimate branch lengths except on a set of measure zero, and we provide extensive empirical results for branch length bias in this setting.
\end{abstract}
...
We show that, when the topology is known and fixed, joint inference for branch length estimation cannot be consistent except on a set of measure zero, such as in cases of zero or infinite branch length, and we find similar areas through empirical means where joint inference consistently underestimates interior branch lengths.
...
%EM Starting here, we only talk about branch lengths and don't mention fidelities, whereas just above we say "estimate a topology and branch fidelities using this"...
%EM At least remind the reader that we're actually talking about fidelities as the variables.
...
%EM Totally confused here!
%EM 1. The generating probabilities do contain x* and y*.
%EM 2. "This is apparent" makes it sound like it should follow directly and that it doesn't require a proof.
...
%EM Is this the right figure reference? It seems strange-- it's about the max ancestral states.
