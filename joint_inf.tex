...
%das: @EM I added iid here but maybe it sounds redundant? I feel it's important and we don't bring up independence until the next section, and only implicitly assume i.d. through \alignmentColumnRV
...
A potential solution in this case using the method of sieves could be to project the column-wise ancestral state patterns into a lower-dimensional space by requiring certain columns to be equal to one another, allowing the degrees of freedom in the ancestral state columns to grow with $\nCols$, albeit more slowly than $O(\nCols)$.
...
%das: my brash intution is that it could potentially be much worse in more complex cases because we have a larger number of degrees of freedom, which is my intuition for why we have inconsistency now. I'm not sure I can justify this though.
