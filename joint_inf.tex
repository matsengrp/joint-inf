...
\begin{figure}
\centering
\includegraphics[width=.9\textwidth]{analytic-inconsistency-legend}
\caption{
...
\begin{align*}
    ...
    &\qquad + p_{13}\cdot\log(1-w').
\end{align*}
At the risk of sounding circular, if $\hat{w}'$ is estimated to be one, then the first likelihood will be chosen, so the region in which that likelihood estimates $\hat{w}'$ to be one will always do so; let's focus on the first likelihood then.
...
The interesting case, to me, is that where $\hat{w}_2' \ge 1$, which is the case where we guarantee $w'$ will be one.
...
When computing the global maximum for $0 \le w' \le 1$ using basin-hopping, we obtain the fit $\hat{w}$ in Fig.~\ref{fig:w_hat}.
We find that the estimate for $\hat{w}$ will typically be an overestimate, and in fact is almost always identically equal to 1, indicating no change along that particular branch.

\begin{figure}
\centering
    \begin{tabular}{cc}
\includegraphics[width=.45\textwidth]{w_hat_heatmap} &
\includegraphics[width=.45\textwidth]{w_hat_heatmap_no_joint}\\
Joint inference & Marginal inference \\
    \end{tabular}
\caption{
    Estimating $\hat{w}$ for $x'$ and $y'$ fixed at their true values.
}
\label{fig:w_hat}
\end{figure}

Second, we consider the more standard case where the true values of $x$ and $y$ are not known.
...
If we estimate $(x', y', w')$ jointly using the same approach above, we obtain the fit $\hat{w}$ in Figure~\ref{fig:w_hat_joint}.
...
We no longer consistently overestimate $\hat{w}$, but the estimates are surprisingly unstable, with estimates of 1 for $\hat{w}$ scattered throughout the $(x,y)$ lattice.
In summary, even given the correct Farris topology we obtain poor estimates for the value of the interior branch length.
