...
For branch length estimation on the correct topology, we outline similar cases where branch length estimates are consistently and heavily biased and provide extensive empirical results for this setting.
\end{abstract}
...
To do so, we use a binary symmetric model with data being generated on the four-taxon ``inverse Felsenstein'' tree (a.k.a.\ the ``Farris'' tree) and we construct bounds on the joint objective function to demarcate a sizeable area of branch lengths in which joint inference is guaranteed to give the wrong tree in the case of perfect sequence data with an infinite number of sites by estimating one or more branch lengths to be zero.
...
We parameterize branches on the unique unrooted four-tip phylogenetic tree in ways known as the ``inverse Felsenstein (InvFels)'' tree (Fig.~\ref{fig:farris-fels-top}).
...
In the standard configuration of this tree, the interior branch length parameters are equal to the bottom two parameters.
We use this standard configuration as our data generating process, though we do not constrain our branch lengths to be equal in considering our optimization objective function.
...
\end{subfigure}
\caption{Two four-taxon trees with fidelities as labeled.
}
...
\end{equation}
In general, the functional form of \eqref{eq:profile_likelihood} is determined by inequalities arising from taking maxima over ancestral state splits in Table~\ref{tab:farris_likelihoods} to obtain the likelihood term for each most likely ancestral state, these terms depending on the unknown $(\tau, t)$.
...
In words, this implies an inconsistency as we converge on the incorrect topology, estimating the interior branch length to be zero.
For example, the Felsenstein tree $\tau_2$ will achieve the same objective function value in this case as the InvFels tree $\tau_1$, as if the interior branch is zero-length these two topologies are indistinguishable.
%TODO: more discussion on proof, intution
...
This is apparent in the ``long $x^*$, short $y^*$'' scenario as these are the cases in which $\emptyset$ is the most likely ancestral state split (Table~\ref{tab:likelihoods} in the Appendix with $x_1=x_2=x^*$ and $y_1=y_2=y^*$).
If we allow multifurcating trees in our inference, then we can think of this as an instance of converging to the wrong topology.
...
To determine how conservative, we use the well-known L-BFGS-B method to perform joint estimation (Fig.~\ref{fig:bl-general-inconsistency}).
We see that the region of inconsistency in the general case is much larger than in the analytic case---the analytic case dealt with loose bounds and only concerned a single set of maximal ancestral states, so this is reasonable.
This region encompasses almost half of the branch parameter space; even given the correct topology and performing our best possible optimization, we have many situations where we will estimate the interior branch length to be zero.

We provide a full description of our optimization procedure in the Appendix, but briefly, we perform 81 maximizations for each separate likelihood function in Table~\ref{table:likelihoods}.
...
We compute these maxima over a lattice in steps of $10^{-2}$ for $x^*$ and $y^*$ from $0$ to $1$.
...
Due to the discontinuities in the fit (Fig.~\ref{fig:bl-general-inconsistency}), an overall estimate of the bias is misleading, and these short-branch cases should be the best settings for joint optimization.
...
Specifically, in the case of four-taxon trees with infinite data, we have obtained nontrivial regions of generating parameters that result in a type of inconsistency: the joint inference procedure estimates severely biased branch lengths even given the correct topology.
...
%TODO: add plot or at least discussion of intuition here
