...
%EM I think this is punchier. Thoughts?
%das: <3 <3
...
%das: is "perfect" a standard term in phylogenetics? Does it just mean infinite length with independent sites?
Although \cite{Sagulenko2017-jo} explicitly warn that their approximation is for the case of branch lengths $\ll 1$, their work motivates understanding when exactly this approximate technique breaks down, even when sequence data is ``perfect''.
...
\caption{
    ...
    Due to the looseness of the upper and lower bounds, the parameters in the non-shaded region do not necessarily indicate consistency, though all parameters in the labeled region result in an inconsistency.
}
...
These situations are in contrast to the approach using marginal likelihood where $\hat{w}$ necessarily converges to $y^*$ as the number of observations grows.
...
We fix bounds on these estimates and show that, in the general case, we either estimate $\hat{w}$ to be one or fall outside of specified bounds for $\hat{x}$ or $\hat{y}$, indicating a poor estimate in at least one of the three unknown branch parameters.
...
Although general forms for $\gamma_L$, $\gamma_U$, and $\beta_L$ are difficult to obtain, we can make progress by bounding the error in branch length estimation for the pendant and/or the central branch.
If we don't want to allow any error in pendant branches, we can use the tightest possible bounds $\gamma_{L} = \gamma_{U} = \gamma$ and $\beta_{L} = \beta$, which is the restricted case of the previous section and given in Fig.~\ref{fig:bl-inconsistency}.
...
In this case, joint inference will either estimate $\hat{w}$ to have fidelity 1 or estimate either $\hat{x}$ or $\hat{y}$ to be more than $0.1$ away from its true value.
...
Direct numerical optimization confirms our theoretically-derived bounds and provides more detailed picture compared to the conservative analytically-derived region of Fig.~\ref{fig:bl-loose-inconsistency}.
...
%EM This could be considered a little unfair. Optimization of 3 variables is harder than optimizing two, so when there is an optimization problem with the 3-variable case then w=1 has an advantage. I don't have a suggestion to change things.
%das: I sort of gave a justification for this in the appendix (line 998); I agree it may not be the most fair, but other than also including a plot of just the 3-variable case (which recall doing at some point but not how it looks) then I'm not sure what else we could say either. Maybe I'll just run the 3-variable case now to have it in our pocket just in case?
...
%EM I'm pretty sure I can see that there's bias in the estimates even in the non-white area. A plot of the difference between the two would be cool. Can we zoom into the region with x* and y* greater than 0.6 so that the scale doesn't get thrown off by big differences where x* and y* are zero?
...
For joint inference, the estimates for $\hat{w}$ when $x^*$ and $y^*$ are both large look reasonable, with $\hat{w}$ increasing as $y^*$ increases, though Fig.~\ref{fig:bl-general-bias} shows there is a systematic positive bias in this procedure when $\hat{w}$ is not estimated to be 1.
...
%EM Given the above, do you think that the next sentence forms a meaningful comparison? The shapes aren't _so_ similar, really...
%das: Yah, I think I was just staring at clouds. Removed.
...
%EM Any intuition of if the effect will go away on larger trees? What about more complex models?
