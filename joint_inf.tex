...
\begin{equation}
%EM Seems a little strange to give a new equation number to an equation that already exists with an assigned number.
...
and
$$
\{\hat{t},\hat{\fullAncestralSplitPartitions}(\tau,t)\} = \argmax_{t,\fullAncestralSplitPartitions(\tau,t)} \ \ell^p_{\tau^*,t^*}(\fullAncestralSplitPartitions(\tau,t),\tau,t).
$$
...
We will parameterize these trees \textbf{not with branch lengths}, but rather such that the probability of a substitution on a branch with parameter $x$ is $p_x=1-2x$.
...
%EM definitely. This is a big TODO.
