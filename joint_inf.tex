...
To do so, we use a binary symmetric model and imagine data being generated on the four-taxa ``Farris zone'' \cite{Siddall1998-hq} tree, and we construct bounds on the joint likelihood to demarcate a sizeable area of long branch lengths in which joint inference is guaranteed to give the wrong tree in the limit of infinite data.
...
\end{subfigure}
\caption{Two four-taxa topologies}
...
The estimates for $\hat{w}'$ when $x$ and $y$ are both large look unbiased, with $\hat{w}'$ increasing as $y$ increases, though the region where $\hat{w}'$ is estimated to be one---indicating no change along the interior branch---is similar to that in the restricted case.
