...
\begin{align*}
    ...
    &\qquad + p_{13}\cdot\log((1-w')).
\end{align*}
When computing the global minimum for $0 \le w' \le 1$ using basin-hopping, we obtain the fit $\hat{w}$ in Figure~\ref{fig:w_hat}.
...
\begin{figure}
\centering
\includegraphics[width=.9\textwidth]{w_hat_heatmap}
\caption{
...
Since we do not know the true values of $x$ and $y$, standard methods will not obtain the situation in Figure~\ref{fig:w_hat}.
The form of the likelihood will alter the fitted values of $x$ and $y$ so that even with perfect information we will not necessarily obtain the true parameters back.
