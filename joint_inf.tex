...
We begin by fixing some notation needed to be precise about the notions of likelihood examined in this paper.
We assume the binary symmetric model, namely with a character alphabet $\alphabet=\{0,1\}$ and a uniform stationary distribution.
...
We show in the Appendix that, in the case of the Farris tree, fixing both the top two branch lengths to be equal and the bottom two branch lengths to be equal will obtain the same maximum likelihood estimate as in the case of arbitrary branch lengths.
...
Fidelities have useful algebraic properties, which mean that the generating probabilities using the Hadamard transform have an especially simple form (see either 8.6 of Semple and Steel \cite{Semple2003-em} or \eqref{eq:hadamard_probability} in the Appendix).
...
For a topology $\tau$ and branch lengths $t$ the likelihood given observed ancestral states $\fullAncestralStates$ is
\begin{equation}
...
\end{equation}
to estimate the tree $\tau$ and branch lengths $t$.

An alternative approach used by \cite{Sagulenko2017-jo} does away with the marginalization and directly estimates the maximum likelihood parameters of the fully\hyp{}observed likelihood in \eqref{eq:full_likelihood}.
...
One can then estimate a topology and branch lengths using this profile likelihood:
\begin{equation}
...
Assume $\fullAlignment$ is generated from topology $\tau^*$ and branch lengths $t^*$.
...
Note that this two step thing is known in statistics as profile likelihood \cite{Murphy2000-ry}.
People have worked on its properties.
...
We would like to thank Richard Neher for helpful discussions.
