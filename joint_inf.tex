...
\title{On joint estimation of ancestral states and phylogenetic model parameters}
...
% In Semple/Steel, there are some assumptions that are necessary for some of the results, called M1, M2 and M3.
% M1 and M2 seem definitely necessary, which pretty much just imply x and y can't be zero.
% M3 implies x and y can't be one?
% (das TODO: double check these; currently working from notes and memory)
...
\begin{figure}
\centering
\includegraphics[width=.9\textwidth]{topology-inconsistency-legend}
\caption{
...
We will now consider the problem of branch length estimation on the correct tree using joint ancestral state and branch length estimation.
We will consider two settings on the Farris topology, corresponding to whether some branch lengths are known ahead of time or not.
...
If $x'$ and $y'$ are fixed, by collapsing like terms and ignoring terms with only $x'$ and $y'$, the likelihood in \eqref{eq:farris_likelihood} as a function of $w'$ is proportional to
\begin{align*}
    ...
    &\qquad + p_{13}\cdot\log(1-w').
\end{align*}
Similarly, the other two possibilities for the likelihood are both proportional to
\begin{align*}
    \ell_{\tau_1,\{x,y,y\}}(\tau_1, w'; \fullAncestralSplitPartitions(\tau_1,w'))
    &\propto        p_{\emptyset}  \cdot\log(1+x^2+y^2+4xyw'+x^2y^2) \\
    &\qquad + p_{13}         \cdot\log(1+x^2+y^2-4xyw'+x^2y^2) \\
...
%EM Not sure what's being suppressed here. Sounds naughty.
%das: I meant suppressing terms not involving $w'$, but I guess I already did that...
...
$$
%EM I don't think you mean "any inequality"... what about sin(ell') > 0?
...
\end{align*}
%EM Is this obvious at this point? Couldn't b^2 - 4ac be zero in principle?
%das: only if x or y is zero (though one could be nonzero).
%b^2 - 4ac is always \ge 0 by \alpha_1 > \alpha_2, so b^2 - 4ac can only be zero if b is zero and a or c is zero.
%c is never zero, so this means a must be zero for this to happen, which only occurs if \gamma = 0, i.e., if x or y is zero.
%Added clarification.
...
\begin{figure}
\centering
\includegraphics[width=.9\textwidth]{branch-length-inconsistency-legend}
...
%EM I don't think I follow the thread of argument here-- what's being assumed when? I think you are assuming $\hat{y}' > \hat{x}'$ ?
...
As a first attempt, simplify further to
\begin{align*}
    \ell_{\tau_1,\{x,y,y\}}(\tau_1, \{x',y',w'\}; \fullAncestralSplitPartitions(\tau_1,\{x',y',w'\}))
    &=        p_{\emptyset}  \cdot\log(1+x'^2+y'^2+4x'y'w'+x'^2y'^2) \\
    &\qquad + p_{13}         \cdot\log(1+x'^2+y'^2-4x'y'w'+x'^2y'^2) \\
...
So fix $x'=y'=u$ and get
\begin{align*}
    \ell_{\tau_1,\{x,y,y\}}(\tau_1, u; \fullAncestralSplitPartitions(\tau_1,u))
    &\propto   p_{\emptyset}  \cdot\log(1+6u^2+u^4) \\
...
% das: there might be something with 1+y^2 < 1+y here?
% I'm not sure how specifically it would affect the maximization, and it might be too loose of a bound, but could work to try.
% The first two terms kind of gunk up the works a little.
...
When computing the global maximum for $0 \le x',y',w' \le 1$ using basin-hopping, we obtain the fit $\hat{w}'$ in Fig.~\ref{fig:w_hat_joint}.
The region in which $\hat{w}'$ is estimated to be one---indicating no change along that particular branch---is now much smaller, but still nontrivial.
Additionally, the estimates are surprisingly unstable, with estimates of 1 for $\hat{w}'$ scattered throughout the $(x,y)$ lattice.
Even given the correct Farris topology we have a region of the space of true parameters where we will always estimate the interior branch parameter to be one.

\begin{figure}
\centering
\includegraphics[width=.9\textwidth]{w_hat_heatmap_joint}
\caption{
