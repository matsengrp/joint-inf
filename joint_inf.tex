...
We derive a set of parameters that generate data under a four-taxon tree and result in this method estimating an incorrect topology in the limit of infinite-length sequences.
For branch length estimation on the correct topology, we outline similar cases where branch lengths are consistently and heavily biased.
\end{abstract}
...
In a recent proposal, \cite{Sagulenko2017-jo} suggest using an approximation to ML inference in which the likelihood is maximized jointly across model parameters and ancestral sequences on a fixed topology.
...
That is, if we know $x$ and $y$ and estimate them to be the truth, then, for
$$
...
%das: @EM Joe pointed out that this is in effect another instance of converging to the wrong topology since once the branch length goes to zero we are no longer in a bifurcating tree case. I don't recall if he had more intuition than the above sentence but he seemed to think it made sense. Not sure if you have any thoughts.
