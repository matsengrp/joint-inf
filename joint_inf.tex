...
In this case those additional parameters come in the form of the bases at ancestral sequences.
...
\caption{
    Regions of topological inconsistency in terms of fidelities.
...
\caption{Region of branch parameter inconsistency in terms of fidelities $x=x^*$ and $y=y^*$ that are fixed to their correct values.}
...
Although general forms for $\gamma_L$, $\gamma_U$, and $\beta_L$ are difficult to obtain, we can make progress by bounding the error in branch length estimation for the pendant or the central branch.
...
In this case, joint inference will either estimate $\hat{w}$ to be one or estimate either $\hat{x}$ or $\hat{y}$ to be more than $0.1$ away from its true value.
...
\caption{
    Region of branch parameter inconsistency in terms of fidelities.
...
Direct numerical optimization confirms our theoretically-derived bounds and provides more detailed picture.
The region of inconsistency in Fig.~\ref{fig:bl-loose-inconsistency} is a conservative estimate of where $\hat{w}=1$.
To determine how conservative, we use the method of basin-hopping \cite{Wales1997} to perform joint maximum likelihood estimation using numerical optimization, as described in the Appendix.
