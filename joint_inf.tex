...
We show that, as $\nCols\rightarrow\infty$, there exist values for $x^*$ and $y^*$ such that the value of the likelihood after maximizing using joint inference is greater for the Felsenstein topology ($\tau_2$) than for the true, generating Farris topology ($\tau_1$).
...
As an intuition for the theoretical development, if there is no change along a branch with fidelity $\theta$, the likelihood will more likely include a term of $(1+\theta)$ instead of $(1-\theta)$ for this event, resulting in positive bias of branch fidelities.
