...
%EM Not sure how tau induces an ordered set of tip labels-- are you saying that the tree defines the labels or something about its structure?
Following Section 8.6 of \cite{Semple2003-em}, $\tau$ induces an ordered set of tip labels $\{1,\ldots,\nSiteRows\}$ across sites.
For the binary symmetric model, a realization of $\alignmentColumn_i$ can be described as a subset of indices $\siteSplit\subseteq\siteSplitSet\triangleq\{1,\ldots,\nSiteRows-1\}$ with equivalent characters, commonly called a ``site split.''
Indeed, we can start by including those tips in $\siteSplit$ that are assigned a 1 in the $\alignmentColumn_i$. Then, because the probability of observing a particular site split is equivalent to the probability of its complement under the binary symmetric model, we can take the complement if needed to exclude tip $\nSiteRows$ from $\siteSplit$.
...
With this split formulation, define the mappings from patterns to splits as described above
$$
...
$$
% the following seemed not necessary and a little confusing (what are j and k?)
% so that, for
% $$
% \patternToSplit(\alignmentColumn_i)=\siteSplit_j, \ \ancestralToSplit(\ancestralStateColumn_i)=\ancestralSplit_k.
% $$
Given a site-pattern-valued random variable $H$ and an ancestral-state-valued random variable $H$, define random variables
$$
...
\begin{align}
L_\nCols'(\fullAlignment;\tau, t) &= \max_{\fullAncestralStates} \ L_\nCols(\fullAlignment;\fullAncestralStates, \tau, t) \\
%EM I guess the ith component of H is h_i?
