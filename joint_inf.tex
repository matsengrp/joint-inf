...
Simplifying and suppressing some variables, this likelihood can be written
$$
...
$$
The inequality $\beta > \gamma$ implies that this function stays finite.
%EM I don't think you mean "any inequality"... what about sin(ell') > 0?
any inequality relating $\ell'$ and 0 is equivalent to the same inequality relating 0 and the quadratic function
\begin{align*}
...
implying $\ell'$ has two zeros according to the quadratic formula.
...
$$
so we are also interested in the inequality $\hat{y}' > \hat{x}'$ if we use the likelihood
\begin{align*}
    \ell_{\tau_1,\{x,y,y\}}(\tau_1, \{x',y',w'\}; \fullAncestralSplitPartitions(\tau_1,\{x',y',w'\}))
    &=        p_{\emptyset}  \cdot\log(1+x'^2+y'^2+4x'y'w'+x'^2y'^2) \\
    &\qquad + p_{1}          \cdot\log(1-x'^2+y'^2-x'^2y'^2) \\
