...
%EM Could we give an extra sentence justifying that the limit of the inferential process will be the same as the inferences applied to the limiting data? Compare https://paperpile.com/shared/HohiVH in which they are rather different.
%das: wow, good catch! This is not at all obvious to me it turns out :/ I guess I'll have to think harder on it
%das: update: I think I've got it, but I need to write it up a little more explicitly, and probably in the appendix. I think this approach maybe doesn't apply to your reference? But I'd like to work through it and see if that's the case, as a sanity check.
Let
\[
...
\]
To show $\hat{t}_\nCols \rightarrow \hat{t}$, we use Wald's consistency proof \citep[p. 48, Theorem 5.14 of ][]{van1998asymptotic}, which requires the following:
\begin{itemize}
\item $\mathcal{T}$ is compact;
\item $t \mapsto m_t(\alignmentColumn)$ is upper-semicontinuous for almost all $\alignmentColumn$;
\item $\alignmentColumn \mapsto \sup_t m_t(\alignmentColumn)$ is measurable;
\item $E[\sup_t m_t] < \infty$.
\end{itemize}
Point one is obviously true.
Point two is true as well: for a fixed ancestral state split $t \mapsto m_t(\alignmentColumn)$ is continuous for all $\alignmentColumn$.
Discontinuities only occur when, if we move about in $\mathcal{T}$, a different ancestral state split becomes more likely.
Maximizing over ancestral state splits ensures the upper-semicontinuous property of this mapping.
For point three, we can think of $\sup_t m_t$ as fixing a maximal ancestral state split.
Since $\alignmentColumn\in\alphabet^\nSiteRows$, the domain $\alphabet^\nSiteRows$ of the mapping $\alignmentColumn \mapsto m_t(\alignmentColumn)$ is a finite set, and so all subsets of the domain are also finite.
This implies measurability of this mapping.
Finally, $m_t(\alignmentColumn)$ is nonpositive for all $t$ and $\alignmentColumn$, satisfying point four.
...
%EM Suggest "Our main theorem states that ... internal branch length zero ..."
Our main theorem states that given data generated on $\tau_1$ there exist true nonzero branch lengths such that the estimator $\hat{t}$ maximizing \eqref{eq:exp-log-lik} has an internal branch of length zero.
...
%EM You're doing some interpretation here, but biologists will skip any paragraph that starts with "proof".
%EM If you are using "topology" to refer to the multifurcating tree structure, you need to clarify that.
...
%EM This is fucking nuts. Nice work. Perhaps drive the point home by adding a sentence repeating that p_w is p_y and just saying the basic point that the entire white region represents inconsistent estimates?
}
\label{fig:bl-general-inconsistency}
\end{figure}

%EM Describe in a sentence how you can reduce things down to 81 maximizations.
...
By the symmetry in the likelihood, we assume without loss of generality that $y_2 \ge y_1$ and $x_2 \ge x_1$, allowing us to consider far fewer potential ancestral state splits than the 3,888 possible given in Table~\ref{tab:farris_likelihoods}.
...
\citet{Sagulenko2018-xl} explicitly warn their approximation is only valid for ``short'' branches, though Fig.~\ref{fig:empirical-bias} shows even when all branches are short there is a consistent bias, and that the bias is on the same order as the magnitude of the parameters.
...
%das: not so sure if your below edit accurately reflects Goldman; he says that, because parsimony (ie, joint inf) and ML estimate different parameters the question is not ineteresting:
%"There has been considerable speculation
%and analysis of when the above methods
%give the same answer (e.g., Felsenstein,
%1973b, 1981b, 1983a; Sober, 1983, 1985; Fel-
%senstein and Sober, 1986; Friday, 1989).
%Now that clear formulations are available
%for each, this question in fact becomes less
%interesting. The methods can now be
%judged on the merits of their models as
%well as on the results they produce; be-
%cause they estimate different parameters,
%the estimates cannot match precisely."
%das: I don't remember reading anywhere him stating that general consistency is an open problem. Regarding general consistency, he says things like "results may be unpredictable" and "may also be inconsistent" and only provides a single example, but this doesn't seem strong enough to say he says it's an open problem. Thoughts?
There, Goldman provides a worked example in which estimating a topology with fixed branch lengths is equivalent to parsimony and thus not guaranteed to be consistent, though he does not discuss the inconsistency of joint inference in general. %EM ... and indeed explicitly describes general consistency as an open problem.
%EM Does the following sentence need revision after the proof updates?
We show cases where the incorrect topology attains an equal likelihood value at the maxium as the correct topology, and, moreover, if we know the correct topology, we show cases where branch lengths are severely biased.
...
%EM Two em-dash phrases in a row is too many.
