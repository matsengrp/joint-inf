...
Consider only the Farris-tree topology.
We will consider two settings, corresponding to whether some branch lengths are known ahead of time or not.
...
If $x'$ and $y'$ are fixed, the likelihood in \eqref{eq:farris_likelihood} as a function of $w'$ is proportional to
\begin{align*}
    ...
    &\qquad + p_{13}\cdot\log(1-w')
\end{align*}
by collapsing like terms and ignoring terms with only $x'$ and $y'$.
...
Since
$$
\log(1+w') >= (1 - p_{13})\cdot\log(1+w') + p_{13}\cdot\log(1-w')
$$
with equality only when $w'=0$, we focus on the second likelihood.
...
$$
The inequality $\beta > \gamma$ implies that any inequality relating $\ell'$ and 0 is equivalent to the same inequality relating 0 and the quadratic function
\begin{align*}
...
By $\alpha_1 > \alpha_2$, $\ell'(0) > 0$ implies $\ell$ is increasing at $w'=0$.
We use this fact to show that, if the smaller of the zeros of $f(w')$ is greater than one, $\hat{w}' \equiv 1$.
