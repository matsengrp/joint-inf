...
For branch length estimation on the correct topology, we show that this joint method cannot estimate consistent branch lengths except on a set of measure zero, and we provide extensive empirical results for outlining the consistent bias in this setting.
\end{abstract}
...
We show that, when the topology is known and fixed, joint inference cannot be consistent except on a set of measure zero, such as in cases of zero or infinite branch length, and we find similar areas through empirical means where joint inference consistently underestimates interior branch lengths.
...
\subsection*{Inconsistent branch length estimation}

When the topology is known and fixed and we estimate only branch lengths, we show the following, i.e., that for almost all $x^*$ and $y^*$ in $(0,1)$ any branch length estimate is consistently biased.
...
In words, the joint estimation procedure never recovers the true generating $t^*$ almost everywhere in the space of generating parameters.
This is apparent given Table~\ref{tab:gen-sitepatprob}, as the solution $\hat{t}$ is a linear combination of $p_{\siteSplit_j}$ values, and no generating probability contains an $x^*$ or $y^*$ term.
...
We give an example in the Appendix following the proof of Theorem~\ref{thm:bl_theorem}.
Thus, inconsistency is guaranteed for almost all $x^*$ and $y^*$, though not all $x^*$ and $y^*$.

\subsection*{Convergence to degenerate topology}
...
%EM A funny thing: this theorem as stated is really under the hypothesis that the ancestral state is emptyset. We can combine this with the lemmas to remove this assumption, but it seems like we should do this for the reader. Time for a corollary?
%das: ok, I reworded it a bit; let me know how you think it sounds.
%EM This is better, but the notation still seems a little funny. Namely, xi is really something that's determined by Y, but the notation seems to hint that it can be specified a priori. The point is that the ML xi has problems, not that there exists a xi that has problems.
%das: ok, edited
...
This result implies an inconsistency as we estimate the interior branch length to be zero (i.e., interior branch fidelity is one) in an open set of values for $x^*$ and $y^*$ (Fig.~\ref{fig:max-anc-state}).
...
%EM I'm going to suggest that we move this to the appendix. The theorem says that there's a set, and we want people to nod then look at the current Figure 3.
%das: good idea; I think this will also appease reviewer 1.
%EM On the other hand we should still refer to it in the text around here.
%das: it's referred to in the first sentence of above paragraph---good?
...
\caption{
    Estimates for $\hat{p}_w=(1-\hat{w})/2$ when optimizing \eqref{eq:profile_likelihood}, where the true value for $p_w$ is $p_{y^*}$.
    Data generated as in Fig.~\ref{fig:max-anc-state}.
...
Inference on the integrated likelihood performs as expected where $\hat{w}$ is equal to $y^*$ regardless of the value of $x^*$ (Fig.~\ref{fig:bl-general-marginal}).
We use L-BFGS-B when optimizing \eqref{eq:marginal_likelihood}.
The errors in this case are lower than machine tolerance showing that, even in cases where joint inference is supposed to do well, it still fails to achieve a low error from truth.
...
Specifically, we have shown there are no sets of generating parameters other than ones of measure zero that yield consistent branch length estimates given the fixed, correct topology.
In addition, in the case of four-taxon trees with infinite data, we have obtained nontrivial regions of generating parameters that result in a type of topological inconsistency: the joint inference procedure estimates zero-length branches, which can be considered as a multifurcating topology.
...
%EM I don't follow here. This doesn't seem like the long branch attraction region of parameter space-- isn't that on the Felsenstein tree, not the InvFels tree?
%das: yeah, it was kind of hand-wavy. edited
