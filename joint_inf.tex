...
$$
%EM It's a little surprising that "naked" beta shows up here. While it seems like a correct statement, why can we have beta here to make the plots, while in another term we have to bound it with beta_L? Seems like we'd need a beta_U.
%das: It's because \alpha_1 and \alpha_2 are still functions of x^* and y^* only, and 2(\alpha_1 + \alpha_2) = 1/2 \beta. The "naked" \beta term comes from the \alphas in the numerator of \ell', while the "clothed" \beta term comes from the \betas in the denominator. I added a sentence in the proof that maybe makes this clear?
