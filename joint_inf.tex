...
%EM This sentence should be made more precise-- it certainly changes the value of the log likelihood-- you mean this restriction does not change the ML.
...
We now show that, in the case of the Farris tree topology, exchanging $x_1$ with $x_2$ and $y_1$ with $y_2$ does not change the value of the likelihood, and fixing both the top two branch lengths to be equal and the bottom two branch lengths to be equal will obtain the same maximum likelihood estimate as in the case of arbitrary branch lengths.
...
Assume we know the true values of $x'$ and $y'$ but not $w'$.
...
\begin{align*}
    ...
    &\qquad + p_{13}\cdot\log((1-w')).
\end{align*}
When computing the global minimum for $0 \le w' \le 1$ using basin-hopping, we obtain the fit $\hat{w}$ in Fig.~\ref{fig:w_hat}.
The estimate for $\hat{w}$ will typically be an overestimate, and in fact is almost always identically equal to 1, indicating no change along that particular branch.
...
Since we do not know the true values of $x$ and $y$, standard methods will not be in the situation in Figure~\ref{fig:w_hat}.
...
Even given the correct Farris topology we obtain poor estimates for the value of the interior branch length.
