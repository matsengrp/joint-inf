...
%Let D(p, q) = \sum_j p_j \log q_j
%This is concave in q_j, trivially, but we are interested in concavity in t.
%Because t generates q_j, how can this not be true?
%Thought:
%- if f(x) has a unique maximum on [a,b] and g(x) is concave on [a,b], all we want to show is that f(x)+g(x) has a unique maximum on [a,b]; moreover, f(x) is concave in an open ball around its maximum.
%- note that we have D(p, q(\theta)), where q(\theta) is this sort of complicated function involving Hadamard matrices. however, in the trivial case, D(p, q(\theta)) is concave, so we may be able to use induction?
%- note f(x) + sum_h g(x,h) has a unique solution; what about f(x) + max_h g(x,h)? we still need concavity...
%- the sum of divergence and partial likelihood may be easy to show is concave as partial likelihood clearly is.
%- empirical results show that the gradient condition is accurate, though that might mean we need to show there are no zero crossings of the derivative or something like that
%Oblique strategies:
%- the condition of y_2 \ge y_1 and x_2 \ge x_1 may play a role
%- we could also find x,y where derivative has no solutions, so fn is always increasing, though this might require some more argument
%- we could show (if it's true) that going from fidelities to the Hadamard representation results in a concave function in t, and from there we know the divergence is concave in t.
%   this seems hard but may be our best bet; proceed by induction seems reasonable, but, again, hard.
%   it's true in the "trivial" n=0 case, which encourages me.
%   moreover, the fidelities generate the probabilities, and the objective is concave in the probabilities, which is more encouragement.
\end{proof}

In words, this means that, if we have a single maximal ancestral state for each site split, we have a unique maximum.
...
%TODO: is there a unique solution? it's not concave, but i think there may be a unique solution; need to show this... gradient argument?
...
%Since x_1=x_2 at the maximum we can use an argument like 1+x^2-2y_1wx > 1+x^2-2x = (1-x)^2 > 0. We'll need to ignore cases where x=0 (i.e., x^* or y^* are zero)
...
\begin{figure}
\centering
\includegraphics[width=.95\textwidth]{positive-gradients-inkscape}
\caption{
