...
An analogous statement holds for $y_2 \ge y_1$, with these likelihoods given in Table~\ref{tab:likelihoods} after maximizing over ancestral states.
...
\max_{t} \ \ell_{\tau^*,t^*}(\tau^*, t) \ge
    ...
    + \tilde{\ell}_{\tau^*,t^*}(\tau^*, \hat{t}).
\]
Fig.~\ref{fig:max-anc-state} shows where the lower bound above is greater than all 80 other upper bounds.
\end{proof}
...
\]
Regardless of $a_{\siteSplit_j} \lessgtr 0$, this is a decreasing function of $x_1$ provided $c_{\siteSplit_j} + a_{\siteSplit_j}x_1 > 0$, but we see
\[
\Pr(\siteSplitRV=\siteSplit_j \mid \tau_1,\{x_1,y_1,x_2,y_2,w\}) := \frac{1}{8} (c_{\siteSplit_j} + a_{\siteSplit_j}x_1)
\]
which is strictly greater than zero provided $x_1,y_1,x_2,y_2\in(0,1)$.
...
\end{align*}
Using the fact that $b_{\emptyset} = -b_{13}$, $b_{1} = -b_{3}$, $b_{2} = -b_{123}$, and $b_{12} = -b_{23}$ in addition to $p_{1} = p_{3}$, $p_{2} = p_{123}$, and $p_{12} = p_{23}$, we simplify the likelihood as
\begin{align*}
...
\end{align*}
Using the bounds on $\hat{x}_1,\hat{y}_1,\hat{y}_1$, and $\hat{y}_2$ from Lemma~\ref{lemma:bounds} and our assumption of $y_2 \ge y_1$ and $x_2 \ge x_1$, we have
\[
0 \le b_{\emptyset} \le \frac{2x^*}{1+(x^*)^2} := b_{\emptyset}^U,
\]
\[
...
\]
Clearly $b_{\emptyset}^U < 1$.
Note that $\ell'_{L}(w)$ is strictly decreasing in $w$ on the interval $(0,1)$, as shown in the previous proof, provided $b_{2}^U, b_{1}^U, b_{12}^U < 1$.
...
So we want $x^*$ and $y^*$ where $b_{2}^U, b_{1}^U, b_{12}^U < 1$ and $\ell_L'(1) > 0$.
These conditions are straightforward, the one on the derivative being
\[
...
\]
The set of $x^*,y^*$ satisfying these conditions as well as those for $\emptyset$ being the most likely ancestral state is given in Fig.~\ref{fig:incons-analytic}.
\end{proof}

As the region in Fig.~\ref{fig:incons-analytic} gives the values of $x^*$ and $y^*$ where $\hat{w}$ is guaranteed to be one, we converge on the incorrect topology in these cases, resulting in an inconsistency.
We show this is a type of inconsistency in topology estimation by constructing a lower bound on the likelihood for the Felsenstein topology and showing it is equal to the likelihood given the InvFels topology in the demarcated region of Fig.~\ref{fig:incons-analytic}.
As we are interested in a lower bound, assume $\ancestralSplitPartition_j(\tau_2,\{x_1,y_1,x_2,y_2,w\})=\emptyset$ for all $j$.
In this case, the partial likelihood term for the Felsenstein tree is composed of the same functional forms given in Table~\ref{tab:likelihoods-restricted}.
This is apparent as assuming $\emptyset$ for the ancestral state split will result in the same partial likelihood for both topologies.

Assume $w=1$.
The maximum likelihood value for the Felsenstein topology is greater than or equal to the value of the likelihood given these two constraints.
In this case, Table~\ref{tab:gen-sitepatprob} shows that, for $w=1$, the divergence term is identical to that of the InvFels topology.
Again, this is apparent as when the internal branch becomes zero-length, the two topologies are indistinguishable.
Let $\mathcal{T}$ be the shaded region in Fig.~\ref{fig:incons-analytic}.
We have
\begin{align*}
\max_{t} & \ \ell_{\tau^*,t^*}(\tau_2, t) \ge \\
&\max_{t} \ \sum_{j=1}^\nSiteSplits \Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*) \cdot \log \Pr(\siteSplitRV=\siteSplit_j \mid \tau_2, \{x_1,y_1,x_2,y_2,1\}) \\
&\qquad + \Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*) \cdot \Pr(\ancestralSplitRV=\emptyset \mid \siteSplitRV=\siteSplit_j, \tau_2, \{x_1,y_1,x_2,y_2,1\}) \\ 
&= \max_{t} \ \sum_{j=1}^\nSiteSplits \Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*) \cdot \log \Pr(\siteSplitRV=\siteSplit_j \mid \tau_1, \{x_1,y_1,x_2,y_2,1\})\\
&\qquad \Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*) \cdot \Pr(\ancestralSplitRV=\emptyset \mid \siteSplitRV=\siteSplit_j, \tau_1, \{x_1,y_1,x_2,y_2,1\})\\
