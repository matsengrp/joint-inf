...
&\qquad \rightarrow \sum_{j=1}^\nSiteSplits \Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*) \cdot \log \Pr(\siteSplitRV=\siteSplit_j, \ancestralSplitRV=\xi_{\siteSplit_j} \mid \tau, t) \\
&\qquad := \ell_{\tau^*,t^*}(\tau, t) \label{eq:site_pattern_profile_likelihood_mean}
\end{split}
\end{equation}
because $\nCols_j(\fullAlignment)/\nCols$ converges to $\Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*)$.
%EM das I believe this is the definition of \ell-- this should be made more explicit.
%EM Also, in the main text we read that \ell is the limit of L' as n goes to infinity, but here it's L''.
%das: L'' is the value that L' is proportional to. It does read kinda awkward, so I just explicitly added the 1/2.
...
Thus exchanging $x_1$ with $x_2$ and $y_1$ with $y_2$ does not change the value of the log-likelihood $\ell_{\tau_1,t}(\tau_1, t)$.
...
\]
where we fix a known value of $\boldsymbol\xi$.
