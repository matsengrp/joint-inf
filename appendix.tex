...
In this proof and other material below, we will use a version of $\ell$ given by
\[
\ell_{\tau^*, t^*}(\tau_1, t; \boldsymbol\xi)
\]
where we fix a given value of the ancestral state split $\boldsymbol\xi$.


\blInconsist*

\begin{proof}
When $\boldsymbol\xi$ is fixed, there exists a closed form solution to $\hat{t} := \{\hat{x}_1,\hat{y}_1,\hat{x}_2,\hat{y}_2,\hat{w}\}$ solving
\[
\hat{t}_{\boldsymbol\xi} = \arg\max_t \ell_{\tau^*, t^*}(\tau_1, t; \boldsymbol\xi).
\]
We first show that this maximum is unique.
