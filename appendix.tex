...
%EM This is very minor compared to the below comments, so address it second in a separate commit, please, if you wish to address it. Curly braces are really for unordered sets, right? I suggest round parensI suggest round parens.
%das: TODO
...
%EM @das I think that ^ is all resolved, right?
%das: I think to be fair I should add that we don't actually show a real inconsistency in topology because we haven't shown this functional convergence for it, only for branch lengths with a fixed topology. I added a paragraph hedging a bit. Let me know what you think.
...
%EM These c_j's and their properties need to be explained a lot more clearly. What are they, how can we know that the likelihood has this form, and why they have the properties below.
%das: heh they must have been carried over---they're actually p_yj's
\begin{align*}
...
Without loss of generality we focus on the variable $x_1$ and the function
\[
...
\begin{align}
%EM Here's this propto thing again. It's up to a constant, not propto, right?
%das: right---reverted to \sim
...
%EM We need to hear why both of these properties hold.
%das: now that c_j is changed to the probability maybe it's more clear? e_j in {0,1} is by definition above
...
%EM Isn't this table redundant given Table S3?
%das: I wrote out the likelihood in the example after the proof
...
\]
is zero.
As the number of zeros of $f_{y^*}(x^*)$ is finite for $0 < x^*, y^* < 1$, consistency holds only on a set of measure zero.
%EM The following sentence falls short of getting us rigorously to where we need to go, which is the measure zero statement. Doesn't that just follow from the number of zeros of a polynomial being finite?
%das: fixed
...
%EM I don't see this paragraph being helpful.
%EM It's a confusing read. First you say that we're going to look at examples, but then perhaps you cast doubt on if the examples exist "There may be no..."
%das: moved some things around so the example is outside of the proof
...
\]
Thus, $\hat{x}_1(x^*, y^*) = x^*(y^*)^2$, and it is possible to obtain $\hat{x}_1(x^*,y^*)=x^*$, such as when $y^*=1$, but we cannot have $\hat{x}(x^*,y^*)_1=x^*$ for all $x^*$ where $0 < x^*, y^* < 1$.
...
\begin{figure}
    \begin{minipage}{\linewidth}
        \centering
        %file: plotting/_output/invfels/ancestral_state_conditions/non-bias/plot.png
...
%EM The rest of this paragraph is obvious, no?
%das: yeah seems like it.

The boundaries determining maximal ancestral state splits (Fig.~\ref{fig:max-anc-state}) are obtained through maximizing over 81 separate functional values (Table~\ref{tab:likelihoods}).
%EM Are you saying that we should be able to obtain this next display equation by maximizing over the entries in Table S3? I think this needs more explanation.
%das: included more detail
...
\]
With similar forms for the remaining variables our likelihood given in~\eqref{eq:site_pattern_profile_likelihood_mean} has an exact, closed form maximum of
\begin{align}
...
\begin{align}
%EM I wouldn't introduce the \ell_{U,1} notation unless it's really needed for some reason I don't see yet.
%das: written as just a function of x* and y*
...
\begin{figure}
\centering
%file: plotting/_output/invfels/marginal_empirical/non-bias/plot.svg
