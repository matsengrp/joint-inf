...
We see
\[
\ell'_{L}(0) = 1 - p_{13}b_{\emptyset}^U - p_{2}b_{2}^U - p_{1}b_{1}^U - p_{12}b_{12}^U > 0
\]
provided $b_{2}^U, b_{1}^U, b_{12}^U < 1$.
Note that $\ell'_{L}(w)$ is strictly decreasing in $w$ on the interval $(0,1)$, as shown in the previous proof.
