...
We define an ``ancestral state split'' in a similar way, where the ancestral state split $\ancestralSplit$ for an internal node $\ancestralStateColumn_i$ is the set of internal nodes labeled with $1$ if the $\nSiteRows$th tip is not labeled with $1$, and as the set of tips labeled with $1$ in $\overline{\ancestralStateColumn}_i$ if the $\nSiteRows$th tip is labeled with $1$
...
%EM "ancestral state splits" are an important item in the development but don't get the same level of definition as "site splits". Let's define them here.
%das: done
...
%EM I think that we should either use "ancestral splits" everywhere, or "ancestral state splits."
%das: yah, agreed
...
\left\{
    \begin{array}{ll}
        \{i'\in\{1,\ldots,\nSiteRows\}: \alignmentColumn_{i,i'}=1\}  & \mbox{if } \alignmentColumn_{i,\nSiteRows}\neq 1,\\
        \{i'\in\{1,\ldots,\nSiteRows\}: \overline{\alignmentColumn}_{i,i'}=1\}  & \mbox{if } \alignmentColumn_{i,\nSiteRows}=1,
    \end{array}
\right.
\]
...
The mapping $\ancestralToSplit$ is defined by whether the tip states have their complements taken or not: if the set of tips labeled $1$ in $\alignmentColumn$ is in $\siteSplitSet$, $\ancestralToSplit(\alignmentColumn, \ancestralStateColumn)$ is the set of tips labeled $1$ in $\ancestralStateColumn$; otherwise, the set of tips labeled $1$ in $\overline{\alignmentColumn}$ necessarily is in $\siteSplitSet$.
...
\end{align*}
where $\overline{Y}$ is the complement of the site-patterned random variable $Y$ and so has the same distribution as $Y$.
Note
\begin{align*}
...
\end{align*}
In other words, for the $j$th site split, $\ancestralSplitPartition_j(\tau, t)\subset\mathcal{P}(\ancestralSplitSet)$ is the set of most likely ancestral state splits for that particular site split, topology and set of branch lengths, and $\xi_j$ is one of possibly many equiprobable ancestral state splits in $\ancestralSplitPartition_j(\tau, t)$.
%EM Wait, does this establish surjectivity? We need to be able to get all values of P(H).
%das: ah crap, I don't know why I wrote the above. I changed a bit of the notation (sorry!) but I hope this will be more clear?
...
This can be seen by using the definition of $\ancestralToSplit(\alignmentColumn_i, \cdot)$ and assuming $\alignmentColumn_{i,i'}\neq 1$, where in this case each of the $2^\nAncestralStateRows$ values of $\ancestralStateColumn$ correspond to each of the $2^\nAncestralStateRows$ elements of $\mathcal{P}(\{1,\ldots,\nAncestralStateRows\})$.
The same can be done for the case of $\alignmentColumn_{i,i'}=1$, implying $\ancestralToSplit(\alignmentColumn_i, \cdot)$ is surjective.
From this we have
\begin{align*}
...
\end{align*}
for some $j$ depending on $i$.
Thus, each term in the likelihood can be collapsed into terms relating only to site splits, indexed by $j$,  as opposed to each individual observation, indexed by $i$.

\subsection*{Site split likelihood}

Thus the likelihood in \eqref{eq:profile_likelihood} can be written as a product over site patterns as opposed to sites:
\begin{align}
...
Table~\ref{tab:gen-sitepatprob} contains calculations of site pattern probabilities for the InvFels topology with arbitrary branch lengths and Table~\ref{tab:sitepatprob} site pattern probabilities for the InvFels topology in Fig.~\ref{fig:farris-fels-top}a.
...
$\siteSplit_j$  & $p_{\siteSplit_j}$ &$\Pr(\siteSplitRV=\siteSplit_j \mid \tau_1,\{x_1,y_1,x_2,y_2,w\})$\\
    ...
    \hline
\end{tabular}
\caption{Site pattern probabilities $p_{\siteSplit_j}$ on the InvFels tree $\tau_1$ with $t=\{x_1,y_1,x_2,y_2,w\}$ obtained using the Hadamard transform.
...
$\siteSplit_j$  & $p_{\siteSplit_j}$ &$\Pr(\siteSplitRV=\siteSplit_j \mid \tau_1,\{x,y,x,y,w\})$\\
    ...
    \hline
\end{tabular}
\caption{Site pattern probabilities $p_{\siteSplit_j}$ on the InvFels tree $\tau_1$ with $t=\{x,y,x,y,w\}$ obtained using the Hadamard transform.
All values multiplied by $1/8$.}
...
Likelihood computations for all site patterns and ancestral states are in Tables~\ref{tab:farris_likelihoods} (for the InvFels tree) and~\ref{tab:fels_likelihoods} (for the Felsenstein tree).
...
& \multicolumn{2}{|c|}{$\ancestralSplit_k$}\\
    ...
    \hline
\end{tabular}
\caption{Likelihood calculations for all site patterns $\siteSplit_j$ and internal states $\ancestralSplit_k$ of the InvFels tree $\tau_1$.
...
\begin{tabular}{|l|ll|}
    ...
    \hline
\end{tabular}
\caption{Likelihood calculations for all site patterns $\siteSplit_j$ and ancestral state partitions $\ancestralSplitPartition_j$ after maximizing over ancestral states on the InvFels tree $\tau_1$ and the Felsenstein tree $\tau_2$.
All values multiplied by $1/32$.
...
Consider the InvFels tree with arbitrary fidelities, i.e., $\tilde{t}=\{x_1,y_1,x_2,y_2,w\}$.
...
%EM I'm thinking about this wrong for some reason: the sum of two things, each of which has a unique maximum, does not necessarily have a unique maximum.
%das: I added the table of general generating probabilities, but it's pretty straightforward I think to see below the second derivative of the likelihood will be negative on the whole interval, implying we have a unique maximum.
%I think we're good.
%Let me know if I should say anything about the continuity of the likelihood, but I think it's obvious this gets us to where we need to be?
...
This implies the above second derivative is negative for each component of $\shannonDivergence_{\tau^*,t^*}(\tau^*,\tilde{t})$, and the second derivative of $\shannonDivergence_{\tau^*,t^*}(\tau^*,\tilde{t})$ itself is negative.
%EM There's not good notational overlap between Table S2 and the definition of partial log-likelihood. Could you fix that?
%das: I think fixed, or did you mean something in addition to changing h-->\Xi and y-->\Psi?

We can compute the partial log-likelihood $\tilde{\ell}_{\tau^*,t^*}(\tau^*, t)$ as in Table~\ref{tab:farris_likelihoods}.
...
\end{align}
%EM Shouldn't this read "obtaining an upper bound"?
%das: yes
...
%das: I don't think so---the first two lines starting at the equals sign are bringing together the first four lines starting above at the <= sign.
%Should I remove this intermediate equation?
%I thought it would help clarify but maybe not?
+ (p_{2}+p_{12}+p_{23}+p_{123})\cdot\log(1-y) \\
...
%EM Let's be more specific, giving an equation number, rather than just saying "the upper bound" here.
%das: is this better? there were a few other typos, so I changed "site split" to "ancestral state split" where needed and updated the constant definitions
...
%das: we never actually use the Gibbs inequality trick for the upper bound---just some log-of-polynomial bounds
In general, taking
\[
