...
\]
%EM Curious: is this an established use of the term "partial"?
%das: not really; a partial likelihood is if the likelihood is separable in the parameters, so more accurate terms for what we're doing might be "marginal likelihood" and "conditional likelihood." I changed a lot throughout, using "integrated likelihood" so we don't confuse "marginal inference" with "marginal likelihood".
...
%EM It seems to me that the point here is that these swaps in the generating probabilities are matched by swaps in the likelihoods, meaning that things stay the same. A topic sentence to this effect would help if that's right.
%das: updated
...
\max_{t} \ \ell_{\tau^*,t^*}(\tau, t; \boldsymbol\xi) \ge
    ...
    + \ell^C_{\tau^*,t^*}(\tau, \tilde{t}; \boldsymbol\xi).
\]
%EM C_0 needs a better definition here. Too much going on.
%das: :check:
...
\]
i.e., the lower bound given $\emptyset$ as the maximal ancestral state split for all site splits and $\tau=\tau_1$.
...
\]
%EM Also suggest a real def for C_{1,u}.
%das: :check:
...
\]
implying $w=y^*$.
Thus the unique maximum is $\hat{t}=\{x^*,y^*,x^*,y^*,y^*\}$, and
\[
