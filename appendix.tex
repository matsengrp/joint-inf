...
\]
which, being a probability, is greater than or equal to zero, so we focus on those fidelities where $c_{x_1,\siteSplit_j} + a_{x_1,\siteSplit_j}x_1 = 0$.

To ensure each $f_j$ is decreasing, we assume that $x_1,y_1,x_2 < 1$; these are the only such cases where $c_{x_1,\siteSplit_j} + a_{x_1,\siteSplit_j}x_1 = 0$ for some $j$.
In fact, since $0 < x^*, y^* < 1$---implying $p_{\siteSplit_j} > 0$ for all $j$---if $c_{x_1,\siteSplit_j} + a_{x_1,\siteSplit_j}x_1 = 0$ for any $j$, we obtain a zero value for our likelihood, i.e., $\shannonDivergence\rightarrow -\infty$, and so the likelihood is never maximized when $c_{x_1,\siteSplit_j} + a_{x_1,\siteSplit_j}x_1 = 0$
Thus, each $f_j$ is decreasing no matter the value of $x_1,y_1,$ or $x_2$, though for some values of $x_1,y_1,$ or $x_2$ equal to one the log-likelihood diverges, and the gradient of the marginal log-likelihood is a decreasing function in $x_1$.
...
%EM Are we saying that Lemma 3 applies in this case? I think so but it could be more specific.
%das: I'm not quite sure I follow the question; I reworded it though because I think it was confusingly stated.
