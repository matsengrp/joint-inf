...
\begin{align*}
    ...
    &= \Pr(\alignmentColumnRV=\alignmentColumn_i \cup \overline{\alignmentColumnRV}=\alignmentColumn_i, \ancestralStateColumnRV=\ancestralStateColumn_i \mid \tau, t)\\
    &= \Pr(\alignmentColumnRV=\alignmentColumn_i, \ancestralStateColumnRV=\ancestralStateColumn_i \mid \tau, t) + \Pr(\overline{\alignmentColumnRV}=\alignmentColumn_i, \ancestralStateColumnRV=\ancestralStateColumn_i \mid \tau, t)\\
...
Since
\begin{align*}
    \Pr(\alignmentColumnRV=\alignmentColumn_i, \ancestralStateColumnRV=\ancestralStateColumn_i \mid \tau, t) &= \Pr(\siteSplitRV=\patternToSplit(\alignmentColumn_i), \ancestralSplitRV=\ancestralToSplit(\alignmentColumn_i, \ancestralStateColumn_i) \mid \tau, t),
\end{align*}
...
From this we have
\begin{align*}
    \max_{\ancestralStateColumn_i} \ \Pr(\alignmentColumnRV=\alignmentColumn_i, \ancestralStateColumnRV=\ancestralStateColumn_i \mid \tau, t) &= \max_{\ancestralStateColumn_i} \ \Pr(\siteSplitRV=\patternToSplit(\alignmentColumn_i), \ancestralSplitRV=\ancestralToSplit(\alignmentColumn_i, \ancestralStateColumn_i) \mid \tau, t)\\
...
We begin with the branch length inconsistency.
...
Applying similar arguments to the other variables, the Hessian for the log-likelihood has nonpositive diagonal terms and off-diagonal terms equal to zero.
...
The true branch fidelity for $x_1$ is $x^*$, and the only cases when we might have $\hat{x}_1=x^*$ are when $y^*=1$ or when $(x^*)^2=x^*$, i.e., one of the generating branch parameters is zero or infinite length; the same is true for $x_2$.
A similar argument for $y_1,y_2,$ and $w$ shows that estimates can only be consistent when $(y^*)^2=y^*$, i.e., $y^*=0$ or $y^*=1$.
\end{proof}

We now proceed to show there exist $x^*$ and $y^*$ such that the interior branch parameter $w$ is estimated as exactly one.

\topoInconsist*

\begin{proof}
As we have a closed form solution to our likelihood problem, we compute the optimal solution given Table~\ref{tab:likelihoods}.
Let
\[
...
We need only consider the possibilities for choices of ancestral state splits in Table~\ref{tab:likelihoods} upon excluding cases of infinite branch lengths (i.e., any of $x_1,y_1,x_2,y_2,w$ equal to zero) and the redundant cases of $x_1 > x_2$ and $y_1 > y_2$, i.e.,
\[
\hat{\boldsymbol\xi} =
...
\begin{figure}
    ...
    \begin{minipage}{0.95\linewidth}
        ...
        \begin{tabular}{ll}
        \hline
        $\boldsymbol\xi_1$ & $\{\emptyset, \emptyset, \emptyset, \emptyset, \emptyset, \emptyset, \emptyset, \emptyset\}$\\
        $\boldsymbol\xi_2$ & $\{\emptyset, \emptyset, \emptyset, \emptyset, \{1,2\}, \emptyset, \emptyset, \emptyset\}$\\
        $\boldsymbol\xi_3$ & $\{\emptyset, \emptyset, \emptyset, \emptyset, \{1,2\}, \emptyset, \emptyset, \{1\}\}$\\
        $\boldsymbol\xi_4$ & $\{\emptyset, \emptyset, \emptyset, \emptyset, \{1,2\}, \emptyset, \{1,2\}, \{1\}\}$\\
        $\boldsymbol\xi_5$ & $\{\emptyset, \emptyset, \emptyset, \{1\}, \{1\}, \emptyset, \{1\}, \{1\}\}$
        \end{tabular}
    \end{minipage}
\caption{
Region with maximal ancestral state splits on the InvFels tree $\tau_1$.
}
\label{fig:max-anc-state}
\end{figure}

Mapping each maximal ancestral state split to each likelihood value, we see that $\hat{w}\equiv 1$ if $\boldsymbol\xi=\boldsymbol\xi_1$ or $\boldsymbol\xi=\boldsymbol\xi_2$, which encompasses the bottom-right region of Figure~\ref{fig:max-anc-state}.
This is an analytically-derived region of inconsistency in terms of probabilities of a character change along a branch for ``perfect'' data generated on the InvFels topology (Fig.~\ref{fig:farris-fels-top}) with $p_{w^*} = p_{y^*}$ (in terms of fidelities, $w^*=y^*$).
\end{proof}
...
Let $\mathcal{T}_0$ be such that, for $t^*=\{x^*,y^*,x^*,y^*,y^*\}$, $t^*\in\mathcal{T}_0$ corresponds to $x^*$ and $y^*$ falling in the shaded region in Fig.~\ref{fig:max-anc-state}.
...
&= \max_{t: \boldsymbol\xi=\boldsymbol\xi_0, w=1, \tau=\tau_1} \ \Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*) \cdot \Pr(\siteSplitRV=\siteSplit_j, \ancestralSplitRV=\xi_j \mid \tau, \{x_1,y_1,x_2,y_2,w\})\\
&= \max_{t: \boldsymbol\xi=\boldsymbol\xi_0, w=1, \tau=\tau_2} \ \Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*) \cdot \Pr(\siteSplitRV=\siteSplit_j, \ancestralSplitRV=\xi_j \mid \tau, \{x_1,y_1,x_2,y_2,w\}).
\end{align*}
