...
\end{tabular}
\caption{
%EM Here and below, it seems a bit funny to call these likelihoods, as the Xi aren't observed.
%das: went with "conditional likelihood"
...
As we exclude the infinite branch length cases by letting $x_1,y_1,x_2,y_2,w > 0$, we have the strict inequality $c_{x_1,\siteSplit_j} + a_{x_1,\siteSplit_j}x_1 > 0$.
Thus, regardless of $a_{x_1,\siteSplit_j} \lessgtr 0$, $f_j$ is a decreasing function of $x_1$.
...
\]
%EM Where is this shown in the previous proof?
%das: wrote out more explicitly. It's similar to the argument starting at L612.
...
%EM I agree that this condition shows that the lower bound is increasing on the entire interval. But how do we get to a conclusion about $\ell(.)$ itself?
%das: The derivative of \ell, i.e., \ell', is a decreasing function on [0,1]. The value of \ell' at 1 is positive, thus, \ell' > 0 on all of [0,1]. Since \ell' > 0 on all of [0,1], \ell is increasing on all of [0,1] (I think via MVT?).
...
\[
%EM Hm... it seems like we need to exclude having the b's very close to 1. How do we do that?
%das: I thought that was taken care of by the condition above: $b_{2}^U, b_{1}^U, b_{12}^U < 1$. It should also be taken care of by the below inequality, too, right? Otherwise as b -> 1 this will go to -\infty, so the below will fail. Do you think they need to be well-separated from 1 instead of just strictly less than 1?
...
%EM Thoughts here?
%das: I kept the one paragraph showing they're equal, but maybe even that's too much?
