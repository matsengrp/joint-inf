...
Therefore we can reduce the number of candidate likelihoods we need to search by, without loss of generality, assuming $x_2 \ge x_1$ and $y_2 \ge y_1$, with these conditional likelihoods given in Table~\ref{tab:likelihoods} after maximizing over ancestral state splits.
...
There exists an open set of $0 < x^*, y^* < 1$ such that $\emptyset$ is the maximal ancestral state split for all site splits given $\tau=\tau_1$.
...
\]
i.e., the lower bound given $\emptyset$ is the maximal ancestral state split for all site splits and $\tau=\tau_1$.
...
\]
for all $j=1,\ldots,\nSiteSplits$ with $t^*=\{x^*,y^*,x^*,y^*,y^*\}$ and $\tau^*$ being the InvFels tree $\tau_1$.
...
For example, reading from the middle panel of Table~\ref{tab:gen-sitepatprob},
\[
...
\]
Regardless of $a_{x_1,\siteSplit_j} \lessgtr 0$, $f_j$ is a decreasing function of $x_1$ so long as $c_{x_1,\siteSplit_j} + a_{x_1,\siteSplit_j}x_1 > 0$.
We have
\[
...
We next exclude the situation of $c_{x_1,\siteSplit_j} + a_{x_1,\siteSplit_j}x_1 = 0$.

To exclude this situation we assume that $x_1,y_1,x_2 < 1$; if this criterion does not hold we get zero likelihoods (which obviously are not maximal).
In fact, since $0 < x^*, y^* < 1$---implying $p_{\siteSplit_j} > 0$ for all $j$---if $c_{x_1,\siteSplit_j} + a_{x_1,\siteSplit_j}x_1 = 0$ for any $j$, we obtain a zero value for our likelihood, i.e., $\shannonDivergence\rightarrow -\infty$.
Hence, the likelihood is never maximized when $c_{x_1,\siteSplit_j} + a_{x_1,\siteSplit_j}x_1 = 0$ and each $f_j$ is decreasing no matter the value of $x_1,y_1,$ or $x_2$, though for some values of $x_1,y_1,$ or $x_2$ equal to one the log-likelihood diverges, and the gradient of the marginal log-likelihood is a decreasing function in $x_1$.
The remaining variables follow similar arguments.

%EM Could you restructure this paragraph with a topic-sentence statement of the assumption we want to make, then evidence that it's ok? It roves around a good bit.
%das: it was a little confusing; I merged and rearranged a few things.
%EM I made a few further changes. Seem OK?
...
\end{tabular}
\caption{
32 times the maximal conditional likelihood values on the InvFels tree $\tau_1$ where $\emptyset$ is the most likely ancestral state split for each site split.
}
\label{tab:likelihoods-restricted}
\end{table}

We now focus on the conditional log-likelihood.
%EM These "Assume"s feel frightening to the reader. Can we replace with "Under our stated hypothesis"?
%das: got rid of some others in the text, too, that seemed like overkill
%EM "we let $\emptyset$ be the most likely ancestral state split" still seems funny to me. We can't "let" it be something that is defined by being a maximum. How about saying that it is _a_ most likely...
% By Lemma~\ref{lemma:ancestral-state}, $\emptyset$ is a most likely ancestral state split for all site splits.
By Lemma~\ref{lemma:ancestral-state}, we let $\emptyset$ be the most likely ancestral state split for all site splits.
...
%EM Does the zero crossing need additional argument? I'm guessing that you get this from the gradient-is-zero thing from the unique-maximum argument above. But that's just that it's zero when we are doing maximization of all of the variables at once. Here aren't we wanting to make an argument for lots of x_i and y_i?
%das: Yeah, I think how it was written before didn't state it correctly. The zero crossing is in all the variables, and we want to make a statement about all the variables at once (as stated in the lemma). We focus on a single variable at a time for the conditional likelihood (\ell^C) because it is separable in each variable, but the constraints are for the maximization over all variables.
%EM This is definitely an improvement, though I don't know if I get it yet. There is a single point at which we know it's zero. We also know that it's decreasing in w. It seems like if we combine these two facts we could still have a function that isn't positive for all $w \le y^*$ (if the level sets of the function are "tilted").
%EM Also, a "zero crossing" at a single point of a multivariate function doesn't seem like the right terminology. Perhaps "vanishes"?
The gradient of the marginal log-likelihood has a zero crossing at $\{x^*,y^*,x^*,y^*,y^*\}$, and the gradient of the conditional log-likelihood is a function of only $w$.
Moreover, the gradient of the marginal log-likelihood is decreasing in $w$, thus, the addition of the derivative of the conditional log-likelihood makes the gradient of the $w$ component of the full log-likelihood positive for all $w \le y^*$.
...
Since $x^*, y^* < 1$, we see the bounds in Lemma~\ref{lemma:bounds} have the property that $x_1,y_1,x_2 < 1$.
...
Using Lemma~\ref{lemma:ancestral-state}, the objective function is
\begin{align*}
...
From the middle panel of Table~\ref{tab:gen-sitepatprob}, we obtain
\[
...
The open set of $x^*,y^*$ satisfying conditions $C1-C4$ as well as in Lemma~\ref{lemma:ancestral-state} is given in Fig.~\ref{fig:incons-analytic}, giving the region of generating parameters where $\hat{w}\equiv 1$.
\end{proof}
...
It is easy to see that letting $\emptyset$ be the maximal ancestral state split results in the same conditional log-likelihood for $\tau_1$ and $\tau_2$.
