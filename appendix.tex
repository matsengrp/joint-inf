...
This along with the previous equalities imply $x_1=x_2=x^*$.
We use the same process to show that, at the maximum, $y_1=y_2=y^*$, and thus that the divergence term attains a unique maximum over $x_1,y_1,x_2,y_2\in(0,1)$.
...
\[
\tilde{\ell}_{\tau_1,t^*}(\tau_1, \tilde{t}) = \sum_{j=1}^\nSiteSplits c_j\cdot\left(\log h_{j,1}(x_1)+\log h_{j,2}(y_1)+\log h_{j,3}(x_2)+\log h_{j,4}(y_2)+\log h_{j,5}(w)\right).
\]
Due to this additive form, any off-diagonal terms of the Hessian for this function will be zero, so it remains to show that the diagonal terms are nonpositive.
...
\]
Again, by assumption $x_1,y_1,x_2,y_2\in(0,1)$, and we need only $0 \le \sum_j c_je_j \le 1$ so that the diagonal term of the Hessian will be nonpositive.
Since $\sum_j c_j = 1$ and $e_j\in\{0,1\}$, then $0 \le \sum_j c_je_j \le 1$ and $\tilde{\ell}''(x_1) \le 0$.
Applying similar arguments to the other variables, the Hessian for the partial log-likelihood will have nonpositive diagonal terms and off-diagonal terms equal to zero.
Thus, the Hessian of the sum of the divergence and the partial log-likelihood will be negative definite, as the Hessian for the divergence is negative definite, provided there is a single maximal ancestral state split for each site split.
...
For site split $\{1\}$, the four possible forms of the partial likelihood are
\[
...
(1-x_1)(1+x_2)(1+w)(1-y_1)(1-y_2).
\]
We ignore the second function as it will always be less than the first except for the case $y_1=y_2=w=0$.
Comparing the remaining functions reveals that there will be an ambiguity in the form of the likelihood only when either $y_1=y_2=0$, $x_1=x_2=0$, or $w=0$.
We reach the same conclusions doing these comparisons for site splits $\emptyset$, $\{2\}$, $\{3\}$, and $\{1,2,3\}$.
The remaining site splits $\{1,2\}$, $\{1,3\}$, and $\{2,3\}$ require the following argument.
For site split $\{1,2\}$, the four possible forms of the partial likelihood are
\[
(1-x_1)(1+x_2)(1+w)(1-y_1)(1+y_2),
\]
\[
...
(1+x_1)(1-x_2)(1+w)(1+y_1)(1-y_2).
\]
We ignore cases where either $y_1=y_2=0$, $x_1=x_2=0$, or $w=0$, and see that an ambiguity in this maximization will occur when
\[
(1-x_1)(1+x_2)(1-y_1)(1+y_2) = (1+x_1)(1-x_2)(1+y_1)(1-y_2).
\]
Note that this can happen when $x_1=y_2=u_1$ and $x_2=y_1=u_2$ but $u_1 \neq u_2$.
However, when $x_1=x_2=u_1$ and $y_1=y_2=u_2$ the partial likelihood takes the same value.
To resolve this ambiguity, we look to the divergence quanitity and recall it will be maximized when $x_1=x_2$ and $y_1=y_2$.
Thus, the full joint likelihood will obtain a unique maximum, and this maximum will occur when $x_1=x_2$ and $y_1=y_2$.
