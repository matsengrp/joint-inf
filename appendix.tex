...
%das: I was thinking bold for matrices/vectors (here of character data) then a vector of character data maps to a set (site split), and so I didn't bold the set.
...
%EM Is eq:site_pattern_likelihood the best ref? It still has counts. It would be nice to have something that's exactly this before plugging in the values from the tables.
%das: gah, you're right
...
%EM This is the case of $\ancestralSplitPartition_j(\tau_1, t) = \{1\}$. You address the other cases later, but at this point it's not clear that you're going to. Can you introduce with this in mind?
%das: done
\begin{align}
...
The previous upper bound is for the ancestral state split $\{1\}$.
For example, for the ancestral state split $\emptyset$ we see the upper bound will be the same except we lose a $(1+x)^2$ term, gain a $(1-x)^2$ term, lose a $(1-w)$ term and gain a $(1+w)$ term.
...
%das: This is old and I believe the text has been updated to fix it. Can you verify?
