...
We now state the Hadamard representation of site split generating probabilities, following Section 8.6 of \cite{Semple2003-em}.
...
$\siteSplit_j$   &$P(\siteSplitRV=\siteSplit_j \mid \tau_1,t)$&$P(\siteSplitRV=\siteSplit_j \mid \tau_2,t)$\\
    ...
    \hline
\end{tabular}
%EM When I came back to this after the break I found it confusing that we refer to these as p_A below (eg in proof of Theorem 1) but that notation doesn't appear in this table. Can we use it here?
%das: Does L540 not help with this? Are you thinking we should add another column with "Notation" or something?
...
%EM why is 1/2 \le p obvious?
%das: I think it's true, but might take some annoying exposition to show. However, p is clearly between zero and one, which still passes the second derivative test.
...
$$
%EM Do we get $0 \le \tilde{p} \le 1/2$ by just adding together terms in Table 1?
%das: Yah, if you add them it's 1/8 (4-4x^2) for 0<x<1.
we have from \eqref{eq:a_const} that $a_1 \ge a_2$ only if $3 \ge 3\tilde{p}$.
Since $\tilde{p} = 1/8\cdot(4-4(x^*)^2)$, $0 \le \tilde{p} \le 1/2$ implying $\hat{x} \ge 0$.
...
%EM Something's not quite right here. If we take the previous comment as truth we should get a
%EM -\gamma^2(1 + \beta) term which is what I get via independent calculation.
%das: I missed a factor of 1/2 in the \alpha_1+\alpha_2 term. Oops. I think below is right given that.
...
\caption{
    %EM I don't think you mean eq:marginal_likelihood here.
    %das: Ha, you're right.
