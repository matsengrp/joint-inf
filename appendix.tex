...
We show that the likelihood $\ell_{\tau_1,t^*}(\tau_1, t)$ remains unchanged if $x_1$ and $x_2$ are exchanged or if $y_1$ and $y_2$ are.
...
Exchanging $x_1$ and $x_2$ also does not change the value of the log-likelihood $\ell_{\tau_1,t^*}(\tau_1, t)$.
...
As we are interested in a lower bound, assume $\xi_j=\emptyset$ for all $j=1,\ldots,\nSiteSplits$.
...
&\qquad\qquad + \Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*) \cdot \Pr(\ancestralSplitRV=\xi_j \mid \siteSplitRV=\siteSplit_j, \tau, \{x_1,y_1,x_2,y_2,w\})\\
&= \max_{t : t^*\in\mathcal{T}_0} \ \ell_{\tau^*,t^*}(\tau_1, t),
\end{align*}
...
\caption{
    Estimates for $\hat{p}_w$ when computing $(\hat{x}_1, \hat{y}_1, \hat{x}_2, \hat{y}_2, \hat{w})$ using L-BFGS-B optimizing the classical marginal likelihood \eqref{eq:marginal_likelihood} rather than a joint optimization procedure.
}
