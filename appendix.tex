...
%das: I think s_j used to be the site pattern and then \tilde{s}_j became the split? It's still the case for h (no tilde is ancestral state pattern, tilde is ancestral state split). I changed to \tilde{y} to be consistent with this, but mabye we can think of something better?
\begin{align*}
...
%das: We added it to clarify that the likelihood now depends on \xi, which is an element of \eta. It seems dirty to ignore it. At the risk of adding more notation, we could add \hat{H} to the original likelihood? Or just axe the etas...
%EM I think I must be missing something:
%EM The likelihood doesn't belong on the specific choice of \xi. Besides, \xi already appears in eq:site_pattern_likelihood.
%EM Eta is a deterministic function of t and tau, so isn't having it redundant?
%das: The likelihood doesn't depend on the specific choice of \xi, but it does depend on \eta. I don't know, I guess redundancy isn't always the worst thing? I just see it as a tradeoff between how explicit we want to be and how clean. Like, in the first line of eq:site_pattern_likelihood we could have as an argument \hat{H} instead of suppressing it, even though it's totally determined by the inputs as-is. Anyway, that was my justification for including \eta. I'm fine with removing it though if you think it's weird.
%das: I changed all this.
...
$\siteSplit_j$   &$P(\siteSplitRV=\siteSplit_j \mid \tau_1,t)$&$P(\siteSplitRV=\siteSplit_j \mid \tau_2,t)$\\
    ...
    \hline
\end{tabular}
\caption{Site pattern probabilities $\siteSplit_j$ on the Farris tree $\tau_1$ and the Felsenstein tree $\tau_2$ obtained using the Hadamard transform.
...
& \multicolumn{2}{|c|}{$\ancestralSplit_k$}\\
    ...
    \hline
\end{tabular}
\caption{Likelihood calculations for all site patterns $\siteSplit_j$ and internal states $\ancestralSplit_k$ of the Farris tree.
...
\hline
\end{tabular}
\caption{Likelihood calculations for all site patterns $\siteSplit_j$ and internal states $\ancestralSplit_k$ of the Felsenstein tree.
All values multiplied by $1/32$.}
...
\begin{tabular}{|l|ll|}
    ...
    \hline
\end{tabular}
\caption{Likelihood calculations for all site patterns $\siteSplit_j$ and ancestral state partitions $\ancestralSplitPartition_j$.
All values multiplied by $1/32$.
...
%EM I think this following statement could use a little more justification. If we write out a symmetric function f in two variables and take the derivative of f(x+delta, x) = f(x, x+delta) with respect to delta and set it to zero we can see that this must be a stationary point of f. Do we need to say more to ensure that this is a maximum?
%das: I wasn't even thinking derivatives. Let f(x, y) be symmetric so that f(x, y) = f(y, x). If f is maximized at some (x_0, y_0) then f(x_0, y_0) > f(x, y) for all x, y. But then by symmetry f(y_0, x_0) is also a maximum. Now *if* a unique maximum exists, x_0 = y_0 should necessarily be this maximum by the properties defined above, otherwise, f(x_0, y_0) would not be equal to f(y_0, x_0). I think there are probably counterexamples where there is not a unique maximum where this statement isn't necessarily true (trivially, because you'd just say x_0 = y_1 if y_1 is also a maximum and voila) but I don't think this discounts the fact that x_0 = y_0 will still be *a* maximum in this case. In that case, it's reasonable to assume so, in my opinion, especially for the topological inconsistency (we don't use the branch length values), but even for the branch length inconsistency because the case of no unique solution is just as bad if not worse than inconsistency.
%das: Should some or all of that be in the text? Can you see anything wrong with it?
%EM I don't see why this works. In fact, imagine that x=y is a ridge of minima, and that we have maxima at (1,2) and at (2,1). My argument shows that we must have a stationary point at x=y, but that doesn't mean that it has to be a maximum!
%das: Wouldn't the assumption of there existing a unique maximum preclude your case? My argument is that if there is a unique maximum, it necessarily has to have x=y. If there is not a unique maximum then the ML problem is ill-posed. I now don't see why we have to have a unique maximum other than to say we hope the problem is well-posed, but it may not be. Do you have any thoughts on where to take this?
%das: So there are references for identifiability, but I'm a little nervous using them because they're for the usual marginal ML problem. How cagey does it sound to say (as I do below) "here are references for identifiability in the usual case, so either joint inf will be not identifiable (bad) or it (likely?) will be and we can use this (which turns out to be inconsistent later)?" Can you think of a better approach?
%das: some other references I found for identifiability are Rogers 1997, RoyChoudhury 2014...
%das: Okay, how about this: assume we've found a maximum of f(x, y) at (x_0, y_0) where f(x, y) = f(y, x) and we want to show that x_0 = y_0
% We know f(y_0, x_0) = f(x_0, y_0) > f(x, y) for all x, y but we don't know yet if x_0 = y_0.
% We can say that x_0 = y_0 if we know f has a unique maximum, which we can argue for if f has a unique maximum in one of its arguments, i.e., f(x, y) is bounded above and concave in its first argument, for example.
% For the joint inf case, f is clearly bounded above, so it remains to show that f is concave.
% Okay, so the partial likelihood term is of the form a*log(1+x) + b*log(1-x) for a >= b >= 0.
% This is concave for x in [0,1] by the second derivative test:
% f' = a/(1+x) - b/(1-x), f'' = -a/(1+x)^2 - b/(1-x)^2 < 0 for all x in [0,1], dope.
% Now, for the term involving generating probabilities, we know there is a unique maximum by Gibbs inequality, meaning this term is also concave.
% And, don't quote me on this yet, but I *believe* the sum of two concave functions will also be concave (how is it not?).
% QED?
The maximum likelihood procedure in phylogenetics results in identifiable parameter estimation under mild conditions \cite{Chang1996}.
The joint inference procedure may not necessarily result in the same identifiability properties, but we claim the following.
If joint inference obtains unique parameter estimates, then, since exchanging $x_1$ and $x_2$ does not change the value of the log-likelihood $\ell_{\tau_1,t^*}(\tau_1, \tilde{t})$, $x_1=x_2$ at the maximum, with the analogous statement holding for $y_1$ and $y_2$.
The case in which we do not obtain unique parameter estimates results in an ill-posed likelihood problem, and thus by design an inconsistency since multiple maxima exist that will not equal the true, generating parameters.
Given this, we let $x_1=x_2=x$ and $y_1=y_2=y$.
...
The full Farris tree log-likelihood takes one of three values depending on branch lengths, which is due to site split $\{1,3\}$ (see Table~\ref{tab:likelihoods}).
We begin by writing out the case for the ancestral state split $\{1\}$, and show the other two cases follow a similar argument.
Directly substituting calculations from Table~\ref{tab:sitepatprob} and Table~\ref{tab:likelihoods} where $\tau = \tau_1$ in \eqref{eq:site_pattern_profile_likelihood_mean}, the log-likelihood is
\begin{align}
...
\begin{align*}
1-x^2-y^2-x^2y^2 & = (1+x)(1-x)(1+y)(1-y) \\
...
\end{align*}
The $p_{\emptyset}$ term is bounded as
\begin{align*}
...
%EM Yay! Would be even better if you specified which upper bound (eqref) you want us to be looking at... there are many.
...
%EM It would seem helful for the below check to express these a_1' etc in terms of tilde{p}.
%das: not quite sure what this comment means actually.
\begin{equation}
    \begin{aligned}
        a_{1}' &= a_{1}-2p_{13}, \\
        a_{2}' &= a_{2}+2p_{13}, \\
...
\end{equation}
for site split $\{1,2\}$.
%EM This check would make a lot more sense if you put the C^{2}_{\tau_1,\tau_1}(x^*, y^*)  and the other analogous definition first.
%EM It would also be nice to do one of the calculations to get us from a_1 world to a_1' world. Just staring at table S4 doesn't make this obvious.
%das: is this a better explanation or did you want it written out explicitly?
...
Note
$$
(y^*)^2\left(1 - 4x^* + (x^*)^2\right) \le 1 - 4x^* + (x^*)^2;
$$
showing that \eqref{eq:lenny} holds if $x^* \le 2$, which is satisfied by definition.
...
In the Felsenstein case, there are many more optimal internal states depending on branch lengths than in the Farris case (Table~\ref{tab:likelihoods}; $\tau = \tau_2$).
...
$$
%EM Hm, the following doesn't look right to me.
%das: Okay, think I missed the point about 1+w for 1-w that I added back.
%EM No, the lower bound is (1+x)(1-x)(1-w)(1+y)(1-y).
%das: I agree that that's *a* lower bound, yeah. So maybe tell me what's wrong with the following: L is the general likelihood in the joint inf problem for the Felsenstein topology, obtained by doing the usual maximization over ancestral states in tab:likelihoods. Now let's obtain L' via the same process but we've now replaced all (1+w)s in that table with (1-w)s. L \ge L', so then L' is a lower bound for the likelihood L, yes? So looking at, let's call it the lower bound version of tab:likelihoods where everything is a (1-w), maximizing over ancestral states then results in the above inequality. Maybe I'm being careless?
%das: Is this better?
...
$$
and simplifying as functions of $\gamma$ and $\beta$ yields
\begin{equation}
...
We posit that $\hat{w}$ is equal to 1 for when $x^*$ and $y^*$ are small; methods that use local optimization may never see candidate values for large $x$ and $y$ if our initial guess is small, so we perform two maximizations, one with $w=1$ fixed and one where $w$ is estimated.
