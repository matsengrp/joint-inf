...
We restrict our attention to $x_1,y_1,x_2,y_2\in(0,1)$, i.e., cases where $0<p_{\siteSplit_j}<1$ for all $\siteSplit_j$.
Additionally, we exclude the infinite branch length case of $w=0$.
%EM This looks good, but remind us what x^* is here.
%das: fixed
...
We use the same process to show that the divergence has a unique maximum for $y_1$ and $y_2$ at $y_1=y_2=y^*$, and thus the divergence term attains a unique maximum over $x_1,y_1,x_2,y_2\in(0,1)$ with $w \neq 0$.
This implies the Hessian of $\shannonDivergence$ is negative definite.
%EM IIUC we only use this fact to show that we have a _negative_ second derivative at the ^* parameter values. It would be nice to emphasize that fact, which would also keep people from worrying that we're falling into the "sum of two things with unique optima has a unique optimum."
%das: Ah, right.
...
\begin{tabular}{|l|ll|}
    ...
    $\begin{array}{l}
                ...
                \end{array}$&
    $\begin{array}{l}
    %EM I must be confused here-- these aren't the values I get at all!
    % e.g. for this first one everything is 0 except for the first taxon, which is 1. So shouldn't it be
                   %(1-x_1)(1+x_2)(1+w)(1+y_1)(1+y_2)\\ ?
    %das: Ha! I must have written out a different case. Went through everything with a fine-tooth comb and noticed one or two slip-ups but everything is basically the same.
                    (1-x_1)(1+x_2)(1+w)(1+y_1)(1+y_2)\\
                    (1+x_1)(1-x_2)(1-w)(1+y_1)(1+y_2)\\
                    (1-x_1)(1+x_2)(1-w)(1-y_1)(1-y_2)\\
                    (1+x_1)(1-x_2)(1+w)(1-y_1)(1-y_2)
...
We now consider the case where there may be more than one ancestral state split that maximizes the partial likelihood for a site split.
...
Comparing the remaining ancestral state splits reveals that there is an ambiguity in the form of the likelihood only when one of $x_1,y_1,x_2,y_2,$ or $w$ is equal to zero, which are cases we exclude.
For example, in comparing ancestral state splits $\emptyset$ and $\{1\}$ an ambiguity arises when
\[
(1-x_1)(1+x_2)(1+w) = (1+x_1)(1-x_2)(1-w),
\]
which holds either if $x_2=0$ and $w=x_1$ or $w=0$ and $x_1=x_2$.
...
For site split $\{1,2\}$, we ignore cases where any of $x_1,y_1,x_2,y_2,$ or $w$ are zero and see that an ambiguity in this maximization can occur when
\[
...
%das: @EM does the below make sense? I know you were worried earlier about arguing the max of the sum of two things, but I think to resolve this ambiguity it's necessary. Basically, we want to show we get a unique t from (max_t f(t) + (max_h g(t,h))) for t branch lengths and h ancestral states. To do this, if max_h g(t,h) has more than one value we can look at f(t) + max_h g(t,h). I.e., max_h g(t,h) happens if either {x_1=y_2,x_2=y_1} or {x_1=x_2,y_1=y_2}, but f(t)|_{x_1=x_2,y_1=y_2} > f(t)|_{x_1=y_2,x_2=y_1} (where |_{} is "restricted to") so the full likelihood will have a unique maximum, and that's what we're concerned about.
...
It is possible that ancestral state splits $\{1\}$ and $\{2\}$ both maximize the partial likelihood as well, but this case follows a similar argument.
