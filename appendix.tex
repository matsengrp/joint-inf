...
%das: wrote out in more detail; I realize now I maybe took some of these intermediate steps for granted. Effectively we want to look at exchanging xs and ys together as well as separately, because they result in different arguments of symmetry, but still don't change the likelihood value.
The same can be done for the splits $\{2\}$ and $\{1,2,3\}$ by exchanging $y_1$ and $y_2$ independent of what we do with $x_1$ and $x_2$.
...
The same is true of the generating probabilities if we exchange only $y_1$ and $y_2$, except, for the case of the conditional likelihood values, we look at the complement of the most likely ancestral state split.
In other words, the function value for the conditional likelihood also swaps between site splits $\{1,2\}$ and $\{2,3\}$, though the most likely ancestral state split is different.
Now, if we exchange both $x_1$ with $x_2$ and $y_1$ with $y_2$, we see these generating probabilities remain unchanged.
Again, for the conditional likelihood values, we look at the complement of the most likely ancestral state split and see these values also remain unchanged.

Exchanging $x_1$ with $x_2$ and $y_1$ with $y_2$ does not change the value of the log-likelihood $\ell_{\tau_1,t^*}(\tau_1, t; \boldsymbol\xi)$.
