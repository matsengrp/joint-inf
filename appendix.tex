...
It remains to show the converse: if $x_1=x_2$ and $y_1=y_2$ then we obtain a unique estimate of $\tilde{t}$ when performing joint inference maximizing \eqref{eq:profile_likelihood}.
...
Table~\ref{tab:likelihoods} shows that an ambiguity in the form of the partial likelihood occurs in one of four cases:
\[
...
\]
In case 1, the equality implies $w=2x/(1+x^2)$.
Plugging this into Table~\ref{tab:likelihoods}, we see the partial log-likelihood, again suppressing constants with respect to $x$ and the initial $1/32$ constant, is
\begin{align*}
\tilde{\ell}(x) &\propto (2-p_1-p_3-p_{12}-p_{23}-2p_{13})\log(1+x) \\
&\qquad + (p_1+p_3+p_{12}+p_{23}+2p_{13})\log(1-x) \\
&\qquad + 2\log(1+x)-\log(1+x^2).
\end{align*}
Call
\[
c := p_1+p_3+p_{12}+p_{23}+2p_{13}
\]
so that straightforward calculus reveals
\begin{align*}
\tilde{\ell}''(x) &= \frac{4x^2}{(1+x^2)^2}-\frac{4-c}{(1+x)^2}-\frac{c}{(1-x)^2}\\
                  &= \left(\frac{4x^2}{(1+x^2)^2}-\frac{4}{(1+x)^2}\right)+c\left(\frac{1}{(1+x)^2}-\frac{1}{(1-x)^2}\right).
\end{align*}
The term on the right is negative as $c \ge 0$ and $0 < x < 1$.
The term on the left can be written as
\[
\frac{4}{(\frac{1}{x}+x)^2}-\frac{4}{(1+x)^2},
\]
which for $0<x<1$ is negative so that $\tilde{\ell}''(x) \le 0$.
Case 2 follows the same argument but for $y$.
In case 3, the ambiguity implies $x=y$, which yields a partial log-likelihood of the form \eqref{eq:partial-x-form} multiplied by two, and thus has negative second derivative.
Finally, case 4 implies $x=y$ and $w=2x/(1+x^2)$, and a combination of the above arguments shows we again have a negative second derivative.
Thus, for $\tilde{t}=\{x_1,y_1,x_2,y_2,w\}$, there is a unique maximum of $\ell_{\tau_1,t^*}(\tau_1, \tilde{t})$ if and only if $x_1=x_2$ and $y_1=y_2$, yielding our identifiability criteria.

Given the above, with the symmetry of the likelihood we let $x_1=x_2=x$ and $y_1=y_2=y$.
