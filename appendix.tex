...
\subsection*{Identifiability criteria and form of the likelihood}

Consider the InvFels tree $\tau_1$ with arbitrary fidelities, i.e., $\tilde{t}=\{x_1,y_1,x_2,y_2,w\}$.
In order for our maximum likelihood problem to be well-posed, we require identifiability conditions on the branch parameters.
We show that, in the case of the InvFels tree, exchanging $x_1$ with $x_2$ and $y_1$ with $y_2$ does not change the value of the likelihood, and that constraining both the top two branch parameters to be equal and the bottom two branch parameters to be equal during inference yields an identifiable maximum likelihood inference procedure.

We first show that the existence of a unique maximizer implies $x_1=x_2$ and $y_1=y_2$.
...
We now show the converse: if $x_1=x_2$ and $y_1=y_2$ then we obtain a unique estimate of $\tilde{t}$ when performing joint inference maximizing \eqref{eq:profile_likelihood}.
...
Assume $x_1=x_2=x$ and $y_1=y_2=y$ so that $t=\{x,y,x,y,w\}$.
We restrict our attention to $x,y\in(0,1)$, i.e., cases where $0<p_{\siteSplit_j}<1$ for all $\siteSplit_j$.
...
\begin{align*}
\tilde{\ell}_{\tau_1,t^*}(\tau_1, t) &= \sum_{j=1}^\nSiteSplits c_j\cdot\log h_{j,x}(x)+\sum_{j=1}^\nSiteSplits c_j\cdot\log h_{j,y}(y)+\sum_{j=1}^\nSiteSplits c_j\cdot\log h_{j,w}(w).
\end{align*}
...
Without loss of generality we focus on the variable $x$ and the partial log-likelihood proportional to
\[
\tilde{\ell}(x) = \sum_{j=1}^\nSiteSplits c_j\cdot\log h_{j,x}(x).
\]
Doing calculation as in Figure~\ref{fig:example_likelihoods}, each functional form, suppressing constants with respect to $x$ and the initial $1/32$ constant, is
\[
h_{j,x}(x) \propto (1+x)^{e_j}(1-x)^{1-e_j}
\]
for $e_j\in\{0,1\}$, which, simplifying, results in
\begin{align}
\tilde{\ell}(x) &\propto \left(\sum_{j=1}^\nSiteSplits c_je_j\right) \log(1+x) + \left(\sum_{j=1}^\nSiteSplits c_j(1-e_j)\right) \log (1-x)\\
                  &= \left(\sum_{j=1}^\nSiteSplits c_je_j\right) \log(1+x) + \left(1-\sum_{j=1}^\nSiteSplits c_je_j\right) \log (1-x). \label{eq:partial-x-form}
\end{align}
We have the second derivative
\[
\tilde{\ell}''(x) = -\left(\frac{\sum_j c_je_j}{(1+x)^2}+\frac{1-\sum_j c_je_j}{(1-x)^2}\right).
\]
As $x,y\in(0,1)$, we need only $0 \le \sum_j c_je_j \le 1$ to imply the diagonal terms of the Hessian are nonpositive.
Since $\sum_j c_j = 1$ and $e_j\in\{0,1\}$, then $0 \le \sum_j c_je_j \le 1$ and $\tilde{\ell}''(x) \le 0$.
...
We have shown above that, for each ancestral state split, we obtain a unique maximum performing joint maximization of the likelihood.
It may be the case that separate ancestral state splits yield different estimates with the same likelihood value.
As joint inference obtains estimates by maximizing over latent variables, if a single functional form is maximal then there is a single solution that is feasible for the joint optimization procedure to recover.
We now consider the case where there may be more than one functional form that maximizes the partial likelihood over ancestral state splits for a site split.
To show joint optimzation results in a unique maximum, it suffices to consider the cases where an ambiguity in the form of the partial likelihood occurs.
Table~\ref{tab:likelihoods} shows this occurs in one of four cases:
\[
1: (1-x)^2(1+w)(1+y)^2 = (1+x)^2(1-w)(1+y)^2 > (1+x)^2(1+w)(1-y)^2,
\]
\[
2: (1+x)^2(1-w)(1+y)^2 = (1+x)^2(1+w)(1-y)^2 > (1-x)^2(1+w)(1+y)^2,
\]
\[
3: (1-x)^2(1+w)(1+y)^2 = (1+x)^2(1+w)(1-y)^2 > (1+x)^2(1-w)(1+y)^2,
\]
or
\[
4: (1-x)^2(1+w)(1+y)^2 = (1+x)^2(1-w)(1+y)^2 = (1+x)^2(1+w)(1-y)^2.
\]
In case 1, the equality implies $w=2x/(1+x^2)$, and in case 2 $w=2y/(1+y^2)$.
Case 3 implies $x=y$, and, finally, case 4 is a combination of these.
These cases may or may not indicate an identifiability issue with the joint inference procedure, as showing a unique estimate arises in each case is nontrivial.
While we may lose identifiability, the set of parameters for which these equalities hold and, additionally, do not yield a unique maximum will encompass a minority of the parameter space.

Thus, for $\tilde{t}=\{x_1,y_1,x_2,y_2,w\}$, there is a unique maximum of $\ell_{\tau_1,t^*}(\tau_1, \tilde{t})$ if and only if $x_1=x_2$ and $y_1=y_2$ for the majority of the parameter space, yielding our identifiability criteria.
With the symmetry of the likelihood we let $x_1=x_2=x$ and $y_1=y_2=y$.
