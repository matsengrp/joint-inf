...
\begin{definition}
Define the complement of $\alignmentColumn$ as $\overline{\alignmentColumn}$.
...
\left\{
    \begin{array}{ll}
        \alignmentColumn  & \mbox{if } \alignmentColumn\in\mathcal{P}(\siteSplitSet) \\
        \overline{\alignmentColumn}  & \mbox{if } \alignmentColumn\notin\mathcal{P}(\siteSplitSet)
    \end{array}
\right.
\]
...
\left\{
    \begin{array}{ll}
        \ancestralStateColumn  & \mbox{if } \alignmentColumn\in\mathcal{P}(\siteSplitSet) \\
        \overline{\ancestralStateColumn}  & \mbox{if } \alignmentColumn\notin\mathcal{P}(\siteSplitSet).
    \end{array}
\right.
\]
...
For each $\alignmentColumn_i$, $\ancestralToSplit(\alignmentColumn_i, \cdot)$ is surjective as it maps all possible values from $\alphabet^\nAncestralStateRows$ to an element in $\mathcal{P}(\ancestralSplitSet)$, and from this we have
\begin{align*}
...
\end{equation}
for site split $\{1,2\}$.
To bound the case of ancestral state split $\ancestralSplitPartition_j(\tau_1, t) = \emptyset$,
\begin{align*}
&    \ell_{\tau_1,t^*}(\tau_1, t) \\
&\qquad\le      p_{\emptyset}  \cdot\log(2)
+ a_{1}'\cdot\log\frac{2a_{1}'}{a_{1}'+a_{2}'}
...
\end{align*}
and, for the case of ancestral state split $\ancestralSplitPartition_j(\tau_1, t) = \{1,2\}$,
\begin{align*}
&    \ell_{\tau_1,t^*}(\tau_1, t) \\
&\qquad\le      p_{\emptyset}  \cdot\log(2)
+ a_{1}\cdot\log\frac{2a_{1}}{a_{1}+a_{2}}
