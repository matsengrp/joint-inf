...
Since exchanging $x_1$ and $x_2$ does not change the value of the log-likelihood $\ell_{\tau_1,t^*}(\tau_1, \tilde{t})$, $x_1=x_2$ at the maximum.
...
In the Felsenstein case, there are quite a few optimal internal states depending on branch lengths (Table~\ref{tab:likelihoods}; $\tau = \tau_2$).
We can bound these below replacing $1+w$ with $1-w$ in Table~\ref{tab:likelihoods} for $\tau = \tau_2$, as this will result in a lower bound on the maximum, so we proceed with that first.
Then, for site split $\{1,2\}$ we have
$$
...
always, the maximum can be bounded below by $(1+x)^2 (1-w)(1+y)^2$.
