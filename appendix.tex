...
We use basin-hopping \cite{Wales1997} implemented in \texttt{scipy} \cite{Jones2001} to obtain the plots in Fig.~\ref{fig:bl-general-inconsistency}.
...
As is apparent from Table~\ref{tab:sitepatprob}, if $0 < x, y < 1$ and we try to evaluate the case of $x'=y'=w'=1$ we have a likelihood of exactly zero.
Worse still, in Table~\ref{tab:likelihoods} for the Farris topology, in the cases of $\{2,\}, \{1,2\}, \{2,3\},$ and $\{1,2,3\}$, any attempt to evaluate $y'=1$ will similarly yield a zero likelihood.
...
We sidestep these computational issues by restricting ourselves to the region $x,y\in [10^{-3},1-10^{-3}]^2$.

We initialize our optimization procedure at the true branch parameter vector $\{x, y, y\}$.
