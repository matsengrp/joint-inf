...
%EM It seems to me that this only works if we know that \tilde{t} = t^*$. Otherwise the divergence won't have this form.
%das: Heh, yeah oops, that was super sloppy.
%Should be fixed, and a lot more explicit now; thanks for keeping me honest!
%Also there's no restriction on w being not equal to zero or one since we're only interested in x and y, so that was removed, too.
%EM Also, I tried to make the connection between the divergence and g a little more explicit.
Define
\[
g_j(x_1) = \Pr(\siteSplitRV=\siteSplit_j \mid \tau, \{x_1,y_1,x_2,y_2,w\}),
\]
i.e., the generating probability of the $j$th site split as a function of $x_1$.
Recall the divergence
\[
\shannonDivergence_{\tau^*,t^*}(\tau,\tilde{t}) = \sum_{j=1}^\nSiteSplits \Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*)\cdot\log \Pr(\siteSplitRV=\siteSplit_j \mid \tau, \tilde{t}),
\]
which, suppressing notation for clarity, we define as a function of $x_1$ as
\[
...
%EM This falls short of being "clearly"--- I can easily see that we get some log(1+x_1) terms and some log(1-x_1) terms, and that the below follows if the sum of the coefficients for the two terms is < 1. Perhaps add a sentence here?
%das: you're right, I missed a factor of two; hope it's more clear now.
We now focus on the partial log-likelihood term.
Recall the partial log-likelihood takes the form
\[
\tilde{\ell}_{\tau^*,t^*}(\tau, \tilde{t}) = \sum_{j=1}^\nSiteSplits \Pr(\siteSplitRV=\siteSplit_j \mid \tau^*, t^*)\cdot\log \Pr(\ancestralSplitRV=\xi_j \mid \siteSplitRV = \siteSplit_j, \tau, \tilde{t}).
\]
...
where
\[
h_j(x_1) = \Pr(\ancestralSplitRV=\xi_j \mid \siteSplitRV = \siteSplit_j, \tau, \{x_1,y_1,x_2,y_2,w\}).
\]
Using the procedure in Figure~\ref{fig:example_likelihoods} with $\tilde{t}=\{x_1,y_1,x_2,y_2,w\}$, we see each functional form, suppressing constants with respect to $x_1$ and the initial $1/32$ constant, will be
\[
...
Since $\sum_j c_j = 1$ and $e_j\in\{0,1\}$, then $0 \le \sum_j c_je_j \le 1$, thus, $D(x_1) + \tilde{\ell}(x_1)$ is concave as a function of $x_1\in(0,1)$, and there exists a single $x_1\in(0,1)$ maximizing the likelihood.
