...
%das: on second thought, this is using notation like in Semple/Steel. If we change this here, I think we'd have to change almost all curly braces in the document, including the ones that use the Semple/Steel notation. I'm not necessarily opposed, but I think it might be confusing, especially because we use their notation a lot throughout. Thoughts?
...
The likelihood~\eqref{eq:profile_likelihood} can be written as
\begin{align}
...
%EM The previous version switched between only looking at x_1 and the general case. Here I tried to make that more flexible and explicit.
%das: looks good :+1:
\begin{align*}
...
%EM This is much improved, thanks! It seems like we need a little more to complete the proof in the general case. We start the proof hypothesizing a fixed \xi. When do we relax that assumption?
%das: yah, maximizing over xi is straightforward since it takes values on a finite set. I changed a bit of notation so hope it's more clear now.
\end{proof}
...
\]
%EM The following sentence needs revision.... read aloud?
%das: changed
