...
Since we have a finite character alphabet, for a given column $i$ there are a finite number of possible assignments of characters to tips $\alignmentColumn_i$ or internal nodes $\ancestralStateColumn_i$; this results in a simpler likelihood calculation.
...
\end{align*}
where $\overline{Y}$ is the complement of the site-patterned random variable $Y$ and $\overline{Y}\sim Y$.
For ancestral states and for some $\ancestralSplit_k\in\mathcal{P}(\ancestralSplitSet)$
\begin{align*}
...
The likelihood in \eqref{eq:profile_likelihood} written as a product over site patterns as opposed to sites is
\begin{align}
