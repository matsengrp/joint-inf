...
$\siteSplit_j$  & $p_{\siteSplit_j}$ &$\Pr(\siteSplitRV=\siteSplit_j \mid \tau_1,t)$&$\Pr(\siteSplitRV=\siteSplit_j \mid \tau_2,t)$\\
    ...
    \hline
\end{tabular}
\caption{Site pattern probabilities $p_{\siteSplit_j}$ on the InvFels tree $\tau_1$ and the Felsenstein tree $\tau_2$ obtained using the Hadamard transform.
...
For the general case of $\tilde{t}=\{x_1, y_2, x_1, y_2, w\}$, we can compute the partial log-likelihood $\tilde{\ell}_{\tau^*,t^*}(\tau^*, t)$ as in Table~\ref{tab:farris_likelihoods}.
