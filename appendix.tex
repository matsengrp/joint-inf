...
%EM "Multiplied by 1/8"... I think this should be "need to be multiplied by 1/8" or "All values have been divided by 1/8."
...
%EM Do discontinuities occur? In my mind we are taking the maximum of continuous functions, which is continuous.
...
\begin{tabular}{|lll|l|}
\hline
$\siteSplit_j$ & $\ancestralSplitPartition_j(\tau_1, t)$ & $\xi_j$ & $\Pr(\ancestralSplitRV=\xi_j \mid \siteSplitRV=\siteSplit_j,\tau_1,t)$\\
\hline
...
We will show that this implies $t=t^*$.
...
We focus on $x_1$.
%EM I think you mean the divergence term as a function of x_1 is proportional to...
The marginal log-likelihood proportional to $x_1$ can be written as
\begin{align*}
%EM This is a little funky-- I think this is a definition of D(x_1)?
...
\begin{align*}
\frac{d}{dx_1} \shannonDivergence(x_1) = \sum_j p_{\siteSplit_j}a_{\siteSplit_j}\frac{1}{c_{\siteSplit_j} + a_{\siteSplit_j}x_1}
\end{align*}
...
\]
as a function of $x_1$, using Tables~\ref{tab:gen-sitepatprob} and~\ref{tab:likelihoods-restricted}, is
\[
%EM Can you help me understand this? Where did the other terms from \log \Pr(\ancestralSplitRV=\emptyset \mid \siteSplitRV = \siteSplit_j, \tau_1, t) go? They don't disappear with the proportionality-- they will be additive terms, not multiplicative. Or perhaps there's a bunch of cancellation? If the latter, let me know and I'll pull out my paper and pad.
\ell^C(x_1; \boldsymbol\xi_0) \propto \left(\frac{1}{2}+\frac{1}{2}x^*(y^*)^2\right)\log(1+x_1) + \left(\frac{1}{2}-\frac{1}{2}x^*(y^*)^2\right)\log(1-x_1).
\]
...
\begin{proof}
%EM "Proportional" thing again.
The objective function proportional to $w$ is
\begin{align*}
%EM The conditional term is \log(1+w), right? If so, it'd be easier to have that second rather than first so that it corresponds to the order in eq:log_likelihood_simplified
\ell(w) \propto \log(1+w) + \sum_j p_{\siteSplit_j} \log(1 + b_{\siteSplit_j}w)
\end{align*}
% Hm, we didn't do a trick quite like this with the division bit.
...
\begin{align*}
%EM the negative sign                                                             V here doesn't seem right. We get the negative sign from going from p13 to the empty set.
...
%das: @EM did you say you wanted to get rid of these paragraphs? we never show there is a case where Fels > InvFels, just where it's equal...
%EM It doesn't seem necessary to me. If we shrink the internal branch to zero, the two topologies match, right? It just seems like we need a sentence or two expressing that.
