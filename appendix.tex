...
\[
p_{\siteSplit_j} := \frac{1}{8} (c_{\siteSplit_j} + a_{\siteSplit_j}x_1)
\]
and $p_{\siteSplit_j} > 0$ so long as $x^*\in(0,1)$ and $y^*\in(0,1)$.
...
Note the following:
\begin{itemize}
\item $b_{\emptyset} = -b_{13}$, $p_{\emptyset} \ge p_{13}$;
\item $b_{1} = -b_{3}$, $p_{1} = p_{3}$;
\item $b_{2} = -b_{123}$, $p_{2} = p_{123}$;
\item $b_{12} = -b_{23}$, $p_{12} = p_{23}$;
\end{itemize}
so
\begin{align*}
